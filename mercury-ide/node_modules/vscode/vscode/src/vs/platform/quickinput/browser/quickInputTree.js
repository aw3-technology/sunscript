...
import { addStandardDisposableListener, asCSSUrl, append, EventType, prepend, addDisposableListener, isHTMLAnchorElement, isAncestor, $ as $$1 } from '../../../base/browser/dom.js';
...
class QuickInputAccessibilityProvider {
    getWidgetAriaLabel() {
        return ( localize(9964, "Quick Input"));
    }
...
let QuickPickItemElementRenderer = class QuickPickItemElementRenderer extends BaseQuickInputListRenderer {
    ...
    renderElement(node, index, data) {
        ...
        if (buttons && buttons.length) {
            data.actionBar.push(( (buttons.map(
                (button, index) => quickInputButtonToAction(button, `id-${index}`, () => element.fireButtonTriggered({ button, item: element.item }))
    ...
    isItemWithSeparatorVisible(item) {
        return (
             (this._itemsWithSeparatorsFrequency.has(item))
        );
    }
...
class QuickPickSeparatorElementRenderer extends BaseQuickInputListRenderer {
    ...
    isSeparatorVisible(separator) {
        return (
             (this._visibleSeparatorsFrequency.has(separator))
        );
    }
    renderElement(node, index, data) {
        const element = node.element;
        ...
        if (buttons && buttons.length) {
            data.actionBar.push(( (buttons.map(
                (button, index) => quickInputButtonToAction(button, `id-${index}`, () => element.fireSeparatorButtonTriggered({ button, separator: element.separator }))
            ))), { icon: true, label: false });
            data.entry.classList.add('has-actions');
        }
        else {
...
let QuickInputTree = class QuickInputTree extends Disposable {
    ...
    get onDidChangeFocus() {
        return (
             (Event.map(
                ...
                e => ( (e.elements.filter((e) => e instanceof QuickPickItemElement).map(e => e.item))),
                this._store
            ))
        );
    }
    get onDidChangeSelection() {
        return (
             (Event.map(this._tree.onDidChangeSelection, e => ({
    ...
    getCheckedElements() {
        return (
             (this._itemElements.filter(e => e.checked)
                .map(e => e.item))
        );
    }
...
QuickInputTree.__decorator = ( (__decorate([
    memoize
