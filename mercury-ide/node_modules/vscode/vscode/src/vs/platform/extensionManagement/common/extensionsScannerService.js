...
import { ExtensionType, UNDEFINED_PUBLISHER, TargetPlatform, parseEnabledApiProposalNames, BUILTIN_MANIFEST_CACHE_FILE, USER_MANIFEST_CACHE_FILE, ExtensionIdentifierMap } from '../../extensions/common/extensions.js';
...
let AbstractExtensionsScannerService = class AbstractExtensionsScannerService extends Disposable {
    ...
    async scanExtensionsUnderDevelopment(scanOptions, existingExtensions) {
        if (this.environmentService.isExtensionDevelopment && this.environmentService.extensionDevelopmentLocationURI) {
            const extensions = (await Promise.all(( (this.environmentService.extensionDevelopmentLocationURI.filter(extLoc => extLoc.scheme === Schemas.file)
                ...
                const input = await this.createExtensionScannerInput(extensionDevelopmentLocationURI, false, ExtensionType.User, true, scanOptions.language, false , undefined, scanOptions.productVersion ?? this.getProductVersion());
    ...
    async scanDevSystemExtensions(language, checkControlFile) {
        ...
        const result = await Promise.all(( (devSystemExtensionsLocations.map(
            async (location) => this.extensionsScanner.scanExtension((await this.createExtensionScannerInput(location, false, ExtensionType.System, true, language, true, undefined, this.getProductVersion())))
        ))));
...
let ExtensionManifestTranslator = class ExtensionManifestTranslator extends Disposable {
    ...
    async getLocalizedMessages(extensionLocation, extensionManifest, nlsConfiguration) {
        ...
        const reportInvalidFormat = (localized) => {
            this.logService.error(this.formatMessage(extensionLocation, ( localize(445, "Invalid format {0}: JSON object expected.", localized?.path))));
        };
...
let ExtensionsScanner = class ExtensionsScanner extends ExtensionManifestTranslator {
    ...
    async scanExtensionManifest(extensionLocation) {
        ...
        try {
            ...
            if (toFileOperationResult(error) !== FileOperationResult.FILE_NOT_FOUND) {
                this.logService.error(this.formatMessage(extensionLocation, ( localize(446, "Cannot read file {0}: {1}.", manifestLocation.path, error.message))));
            }
            return null;
        }
        let manifest;
