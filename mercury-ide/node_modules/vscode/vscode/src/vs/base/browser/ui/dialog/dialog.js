...
class Dialog extends Disposable {
    ...
    getIconAriaLabel() {
        let typeLabel = ( localize(9522, 'Info'));
        switch (this.options.type) {
            case 'error':
                typeLabel = ( localize(9523, 'Error'));
                break;
            case 'warning':
                typeLabel = ( localize(9524, 'Warning'));
                break;
            case 'pending':
                typeLabel = ( localize(9525, 'In Progress'));
                break;
        }
        return typeLabel;
    }
        ...
        return (
             (new Promise((resolve) => {
                ...
                if (!this.options.disableCloseAction) {
                    ...
                    const action = this._register(( (new Action('dialog.close', ( localize(9526, "Close Dialog")), ThemeIcon.asClassName(Codicon.dialogClose), true, async () => {
                        resolve({
                            button: this.options.cancelId || 0,
                            checkboxChecked: this.checkbox ? this.checkbox.checked : undefined
                        });
