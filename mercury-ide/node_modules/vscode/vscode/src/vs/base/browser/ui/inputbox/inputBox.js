import { append, getTotalHeight, isActiveElement, getTotalWidth, asCssValueWithDefault, $ as $$1 } from '../../dom.js';
import { DomEmitter } from '../../event.js';
import { renderFormattedText, renderText } from '../../formattedTextRenderer.js';
import { ActionBar } from '../actionbar/actionbar.js';
import { alert, status } from '../aria/aria.js';
...
class InputBox extends Widget {
    ...
    _showMessage() {
        ...
        if (this.message.type === MessageType.ERROR) {
            alertText = ( localize(9834, "Error: {0}", this.message.content));
        }
        else if (this.message.type === MessageType.WARNING) {
            alertText = ( localize(9835, "Warning: {0}", this.message.content));
        }
        else {
            alertText = ( localize(9836, "Info: {0}", this.message.content));
        }
        alert(alertText);
        this.state = 'open';
    }
...
class HistoryInputBox extends InputBox {
    constructor(container, contextViewProvider, options) {
        const NLS_PLACEHOLDER_HISTORY_HINT_SUFFIX_NO_PARENS = ( localize(9837, ' or {0} for history', `\u21C5`));
        const NLS_PLACEHOLDER_HISTORY_HINT_SUFFIX_IN_PARENS = ( localize(9838, ' ({0} for history)', `\u21C5`));
    ...
    showNextValue() {
        ...
        this.value = next ?? '';
        status(this.value ? this.value : ( localize(9839, "Cleared Input")));
    }
    showPreviousValue() {
        if (!( (this.history.has(this.value)))) {
            this.addToHistory();
...
export { HistoryInputBox, InputBox, MessageType, unthemedInboxStyles };
