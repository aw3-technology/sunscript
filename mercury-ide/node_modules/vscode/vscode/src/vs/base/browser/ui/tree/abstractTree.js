import { createStyleSheet, isActiveElement, asCssValueWithDefault, isKeyboardEvent, append, $, clearNode, addDisposableListener, hasParentWithClass, h, getWindow } from '../../dom.js';
...
import { isInputElement, isStickyScrollContainer, isStickyScrollElement, List, MouseController, isButton, isMonacoEditor, isMonacoCustomToggle, isActionItem } from '../list/listWidget.js';
import { Toggle, unthemedToggleStyles } from '../toggle/toggle.js';
import { isFilterResult, getVisibleState } from './indexTreeModel.js';
import { TreeMouseEventTarget, TreeError, TreeDragOverBubble, TreeVisibility } from './tree.js';
import { Action } from '../../../common/actions.js';
import { firstOrDefault, equals, distinct, range } from '../../../common/arrays.js';
...
class FuzzyToggle extends Toggle {
    constructor(opts) {
        super({
            ...
            title: ( localize(4962, "Fuzzy Match")),
            isChecked: opts.isChecked ?? false,
            hoverDelegate: opts.hoverDelegate ?? getDefaultHoverDelegate('element'),
            inputActiveOptionBorder: opts.inputActiveOptionBorder,
            inputActiveOptionForeground: opts.inputActiveOptionForeground,
...
class FindWidget extends Disposable {
    set mode(mode) {
        ...
        this.findInput.inputBox.setPlaceHolder(mode === TreeFindMode.Filter ? ( localize(4963, "Type to filter")) : ( localize(4964, "Type to search")));
    }
    ...
    constructor(container, tree, contextViewProvider, mode, matchType, options) {
        ...
        this.modeToggle = this._register(( (new ModeToggle(
            { ...styles.toggleStyles, isChecked: mode === TreeFindMode.Filter, hoverDelegate: toggleHoverDelegate }
        ))));
        this.matchTypeToggle = this._register(( (new FuzzyToggle(
            { ...styles.toggleStyles, isChecked: matchType === TreeFindMatchType.Fuzzy, hoverDelegate: toggleHoverDelegate }
        ))));
        ...
        this.findInput = this._register(( (new FindInput(this.elements.findInput, contextViewProvider, {
            label: ( localize(4964, "Type to search")),
        ...
        const closeAction = this._register(( (new Action('close', ( localize(4965, "Close")), 'codicon codicon-close', true, () => this.dispose()))));
    ...
    render() {
        ...
        if (this.pattern && noMatches) {
            if (this.tree.options.showNotFoundMessage ?? true) {
                this.widget?.showMessage({ type: MessageType.WARNING, content: ( localize(4966, "No elements found.")) });
            }
            else {
...
class StickyScrollController extends Disposable {
    constructor(tree, model, view, renderers, treeDelegate, options = {}) {
        ...
        this._widget = this._register(( (new StickyScrollWidget(
...
class TreeNodeList extends List {
    ...
    createMouseController(options) {
        return (
             (new TreeNodeListMouseController(this, options.tree, options.stickyScrollProvider))
        );
    }
...
class AbstractTree {
    ...
    get onMouseClick() { return (
         (Event.map(this.view.onMouseClick, asTreeMouseEvent))
    ); }
    ...
    get onMouseOver() { return (
         (Event.map(this.view.onMouseOver, asTreeMouseEvent))
    ); }
    get onMouseOut() { return (
         (Event.map(this.view.onMouseOut, asTreeMouseEvent))
    ); }
    ...
    get onTap() { return (
         (Event.map(this.view.onTap, asTreeMouseEvent))
    ); }
    get onPointer() { return (
         (Event.map(this.view.onPointer, asTreeMouseEvent))
    ); }
    ...
    constructor(_user, container, delegate, renderers, _options = {}) {
        ...
        this.view = ( (new TreeNodeList(
            ...
            this.selection,
            this.anchor,
            { ...asListOptions(() => this.model, _options), tree: this, stickyScrollProvider: () => this.stickyScrollController }
        )));
...
export { AbstractTree, AbstractTreePart, AbstractTreeViewState, ComposedTreeDelegate, FuzzyToggle, ModeToggle, RenderIndentGuides, TreeFindMatchType, TreeFindMode, TreeRenderer };
