...
function renderMarkdownAsPlaintext(markdown, withCodeBlocks) {
    ...
    const html = marked_1.parse(value, { renderer: withCodeBlocks ? plainTextWithCodeBlocksRenderer.value : plainTextRenderer.value }).replace(/&(#\d+|[a-zA-Z]+);/g, m => unescapeInfo.get(m) ?? m);
...
function completeListItemPattern(list) {
    ...
    const newListItemText = lastListItemLead +
        ...
        newToken.raw;
    const newList = marked_1.lexer(previousListItemsText + newListItemText)[0];
