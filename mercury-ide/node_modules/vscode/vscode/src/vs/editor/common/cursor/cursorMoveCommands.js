...
import { CursorState, SingleCursorState, SelectionStartKind } from '../cursorCommon.js';
...
class CursorMoveCommands {
    ...
    static expandLineSelection(viewModel, cursors) {
        ...
        for (let i = 0, len = cursors.length; i < len; i++) {
            ...
            result[i] = CursorState.fromModelState(( new SingleCursorState(( new Range(startLineNumber, 1, startLineNumber, 1)), SelectionStartKind.Simple, 0, ( new Position(endLineNumber, endColumn)), 0)));
        }
        return result;
    }
    ...
    static line(viewModel, cursor, inSelectionMode, _position, _viewPosition) {
        ...
        if (!inSelectionMode) {
            ...
            return CursorState.fromModelState(( new SingleCursorState(( new Range(position.lineNumber, 1, selectToLineNumber, selectToColumn)), SelectionStartKind.Line, 0, ( new Position(selectToLineNumber, selectToColumn)), 0)));
        }
    ...
    static simpleMove(viewModel, cursors, direction, inSelectionMode, value, unit) {
        switch (direction) {
            ...
            case CursorMove.Direction.PrevBlankLine: {
                if (unit === CursorMove.Unit.WrappedLine) {
                    return ( cursors.map(
                        cursor => CursorState.fromViewState(MoveOperations.moveToPrevBlankLine(viewModel.cursorConfig, viewModel, cursor.viewState, inSelectionMode))
                    ));
                }
                else {
                    return ( cursors.map(
                        cursor => CursorState.fromModelState(MoveOperations.moveToPrevBlankLine(viewModel.cursorConfig, viewModel.model, cursor.modelState, inSelectionMode))
                    ));
                }
            }
            case CursorMove.Direction.NextBlankLine: {
                if (unit === CursorMove.Unit.WrappedLine) {
                    return ( cursors.map(
                        cursor => CursorState.fromViewState(MoveOperations.moveToNextBlankLine(viewModel.cursorConfig, viewModel, cursor.viewState, inSelectionMode))
                    ));
                }
                else {
                    return ( cursors.map(
                        cursor => CursorState.fromModelState(MoveOperations.moveToNextBlankLine(viewModel.cursorConfig, viewModel.model, cursor.modelState, inSelectionMode))
                    ));
                }
            }
    ...
    static _moveLeft(viewModel, cursors, inSelectionMode, noOfColumns) {
        return ( cursors.map(
            cursor => CursorState.fromViewState(MoveOperations.moveLeft(viewModel.cursorConfig, viewModel, cursor.viewState, inSelectionMode, noOfColumns))
        ));
    }
    ...
    static _moveRight(viewModel, cursors, inSelectionMode, noOfColumns) {
        return ( cursors.map(
            cursor => CursorState.fromViewState(MoveOperations.moveRight(viewModel.cursorConfig, viewModel, cursor.viewState, inSelectionMode, noOfColumns))
        ));
    }
