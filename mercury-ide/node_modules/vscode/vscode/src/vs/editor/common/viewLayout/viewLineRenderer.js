...
function renderViewLine(input, sb) {
    if (input.lineContent.length === 0) {
        if (input.lineDecorations.length > 0) {
            ...
            return (
                 (new RenderLineOutput(characterMapping, false, containsForeignElements))
            );
        }
        sb.appendString('<span><span></span></span>');
        return (
             (new RenderLineOutput( (new CharacterMapping(0, 0)), false, ForeignElementType.None))
        );
    }
...
function _renderLine(input, sb) {
    ...
    if (isOverflowing) {
        ...
        sb.appendString(( localize(9364, "Show more ({0})", renderOverflowingCharCount(overflowingCharCount))));
        sb.appendString('</span>');
    }
    sb.appendString('</span>');
    return (
         (new RenderLineOutput(characterMapping, containsRTL, containsForeignElements))
    );
}
...
function renderOverflowingCharCount(n) {
    if (n < 1024) {
        return ( localize(9365, "{0} chars", n));
    }
    if (n < 1024 * 1024) {
