...
import { observableSignalFromEvent, observableSignal } from '../../../../base/common/observableInternal/utils.js';
    ...
    constructor(model, _options, _diffProviderFactoryService) {
        ...
        this._register(autorun(reader => {
            ...
            const updatedLastUnchangedRegions = ( lastUnchangedRegions.regions.map(
                (r, idx) => (!lastUnchangedRegionsOrigRanges[idx] || !lastUnchangedRegionsModRanges[idx]) ? undefined :
                    ( new UnchangedRegion(
            ...
            if (didChange) {
                const originalDecorationIds = model.original.deltaDecorations(lastUnchangedRegions.originalDecorationIds, ( newRanges.map(
                    r => ({ range: r.originalUnchangedRange.toInclusiveRange(), options: { description: 'unchanged' } })
                )));
                const modifiedDecorationIds = model.modified.deltaDecorations(lastUnchangedRegions.modifiedDecorationIds, ( newRanges.map(
                    r => ({ range: r.modifiedUnchangedRange.toInclusiveRange(), options: { description: 'unchanged' } })
                )));
                transaction(tx => {
        ...
        const updateUnchangedRegions = (result, tx, reader) => {
            ...
            const originalDecorationIds = model.original.deltaDecorations(lastUnchangedRegions?.originalDecorationIds || [], ( newUnchangedRegions2.map(
                r => ({ range: r.originalUnchangedRange.toInclusiveRange(), options: { description: 'unchanged' } })
            )));
            const modifiedDecorationIds = model.modified.deltaDecorations(lastUnchangedRegions?.modifiedDecorationIds || [], ( newUnchangedRegions2.map(
                r => ({ range: r.modifiedUnchangedRange.toInclusiveRange(), options: { description: 'unchanged' } })
            )));
            this._unchangedRegions.set({
        ...
        this._register(model.modified.onDidChangeContent((e) => {
            ...
            if (diff) {
                TextEditInfo.fromModelContentChanges(e.changes);
            }
...
class UnchangedRegion {
    ...
    constructor(originalLineNumber, modifiedLineNumber, lineCount, visibleLineCountTop, visibleLineCountBottom) {
        ...
        this._shouldHideControls = derived(this, reader =>  this.visibleLineCountTop.read(reader) + this.visibleLineCountBottom.read(reader) === this.lineCount && !this.isDragged.read(reader));
...
export { DiffEditorViewModel, DiffMapping, DiffState, RevealPreference, UnchangedRegion };
