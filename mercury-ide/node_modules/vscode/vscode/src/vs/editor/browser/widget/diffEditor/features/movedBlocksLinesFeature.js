...
import { autorun, autorunWithStore, autorunHandleChanges } from '../../../../../base/common/observableInternal/autorun.js';
import { observableFromEvent, observableSignalFromEvent, recomputeInitiallyAndOnChange, constObservable } from '../../../../../base/common/observableInternal/utils.js';
import '../../../../../base/common/cancellation.js';
import { ThemeIcon } from '../../../../../base/common/themables.js';
import { applyViewZones, ViewZoneOverlayWidget, applyStyle, PlaceholderViewZone } from '../utils.js';
import { OffsetRangeSet, OffsetRange } from '../../../../common/core/offsetRange.js';
...
class MovedBlocksLinesFeature extends Disposable {
    ...
    constructor(_rootElement, _diffModel, _originalEditorLayoutInfo, _modifiedEditorLayoutInfo, _editors) {
        ...
        this._register(applyViewZones(this._editors.original, ( (movedBlockViewZones.map(
            zones => (  (zones.map(z => z.original)))
        )))));
        this._register(applyViewZones(this._editors.modified, ( (movedBlockViewZones.map(
            zones => (  (zones.map(z => z.modified)))
        )))));
        this._register(autorunWithStore((reader, store) => {
            const blocks = movedBlockViewZones.read(reader);
            for (const b of blocks) {
                store.add(( (new MovedBlockOverlayWidget(
...
class LinesLayout {
    static compute(lines) {
        ...
        return (
             (new LinesLayout(setsPerTrack.length, trackPerLineIdx))
        );
    }
