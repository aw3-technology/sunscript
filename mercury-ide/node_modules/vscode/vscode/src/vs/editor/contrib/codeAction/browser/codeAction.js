...
import { mayIncludeActionsOfKind, CodeActionTriggerSource, CodeActionKind, filtersAction, CodeActionItem } from '../common/types.js';
...
class ManagedCodeActionSet extends Disposable {
    ...
    get hasAutoFix() {
        return (
             (this.validActions.some(
                ({ action: fix }) => !!fix.kind && CodeActionKind.QuickFix.contains(( (new HierarchicalKind(fix.kind)))) && !!fix.isPreferred
            ))
        );
    }
    get hasAIFix() {
        return (
             (this.validActions.some(({ action: fix }) => !!fix.isAI))
        );
    }
    get allAIFixes() {
...
async function getCodeActions(registry, model, rangeOrSelection, trigger, progress, token) {
    ...
    const excludeNotebookCodeActions = (trigger.type === CodeActionTriggerType.Auto);
    ...
    try {
        ...
        return (
             (new ManagedCodeActionSet(allActions, allDocumentation, disposables))
        );
    }
    finally {
        listener.dispose();
...
function getCodeActionProviders(registry, model, filter) {
    return registry.all(model)
        ...
        return (
             (provider.providedCodeActionKinds.some(
...
function getDocumentationFromProvider(provider, providedCodeActions, only) {
    ...
    const documentation = ( (provider.documentation.map(
        entry => ({ kind: ( (new HierarchicalKind(entry.kind))), command: entry.command })
    )));
    if (only) {
        let currentBest;
    ...
    if (item.action.command) {
        try {
            ...
            notificationService.error(typeof message === 'string'
                ? message
                : ( localize(4521, "An unknown error occurred while applying the code action")));
        }
    }
}
...
CommandsRegistry.registerCommand('_executeCodeActionProvider', async function (accessor, resource, rangeOrSelection, kind, itemResolveCount) {
    ...
    const codeActionSet = await getCodeActions(codeActionProvider, model, validatedRangeOrSelection, { type: CodeActionTriggerType.Invoke, triggerAction: CodeActionTriggerSource.Default, filter: { includeSourceActions: true, include } }, Progress.None, CancellationToken.None);
    const resolving = [];
    ...
    try {
        ...
        return (
             (codeActionSet.validActions.map(item => item.action))
        );
    }
    finally {
