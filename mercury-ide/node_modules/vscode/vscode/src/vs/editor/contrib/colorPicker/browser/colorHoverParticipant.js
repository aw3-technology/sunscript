...
import { getColors, getColorPresentations } from './color.js';
import { ColorDetector } from './colorDetector.js';
import { ColorPickerModel } from './colorPickerModel.js';
import { ColorPickerWidget } from './colorPickerWidget.js';
import { RenderedHoverParts, HoverAnchorType } from '../../hover/browser/hoverTypes.js';
...
let ColorHoverParticipant = class ColorHoverParticipant {
    ...
    getAccessibleContent(hoverPart) {
        return ( localize(5040, 'There is a color picker here.'));
    }
    handleResize() {
...
async function _createColorHover(participant, editorModel, colorInfo, provider) {
    ...
    if (participant instanceof ColorHoverParticipant) {
        return (
             (new ColorHover(participant, Range.lift(colorInfo.range), model, provider))
        );
    }
    else {
        return (
             (new StandaloneColorPickerHover(participant, Range.lift(colorInfo.range), model, provider))
        );
    }
}
function renderHoverParts(participant, editor, themeService, hoverParts, context) {
    ...
    const colorPicker = disposables.add(( (new ColorPickerWidget(
        context.fragment,
        model,
...
export { ColorHover, ColorHoverParticipant, StandaloneColorPickerHover, StandaloneColorPickerParticipant };
