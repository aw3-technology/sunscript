             ...
             (new AsyncIterableObject(async (executor) => {
                ...
                if (isNonEmptyArray(part.item.hint.textEdits)) {
                    executor.emitOne(( (new MarkdownHover(
                        this,
                        anchor.range,
                        [( (new MarkdownString())).appendText(( localize(5175, "Double-click to insert")))],
                        false,
                        10001
                    ))));
                }
                ...
                if (part.part.location || part.part.command) {
                    ...
                    const kb = useMetaKey
                        ? isMacintosh
                            ? ( localize(5176, "cmd + click"))
                            : ( localize(5177, "ctrl + click"))
                        : isMacintosh
                            ? ( localize(5178, "option + click"))
                            : ( localize(5179, "alt + click"));
                    if (part.part.location && part.part.command) {
                        linkHint = ( (new MarkdownString())).appendText(( localize(5180, 'Go to Definition ({0}), right click for more', kb)));
                    }
                    else if (part.part.location) {
                        linkHint = ( (new MarkdownString())).appendText(( localize(5181, 'Go to Definition ({0})', kb)));
                    }
                    else if (part.part.command) {
                        linkHint = ( (new MarkdownString(`[${( localize(5182, "Execute Command"))}](${asCommandLink(part.part.command)} "${part.part.command.title}") (${kb})`, { isTrusted: true })));
                    }
                    if (linkHint) {
