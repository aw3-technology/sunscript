...
import { ClickAction, KeyDownAction, HoverPosition } from '../../../../base/browser/ui/hover/hoverWidget.js';
...
const $ = $$1;
const increaseHoverVerbosityIcon = registerIcon('hover-increase-verbosity', Codicon.add, ( localize(5160, 'Icon for increaseing hover verbosity.')));
const decreaseHoverVerbosityIcon = registerIcon('hover-decrease-verbosity', Codicon.remove, ( localize(5161, 'Icon for decreasing hover verbosity.')));
class MarkdownHover {
    ...
    createLoadingMessage(anchor) {
        return (
             (new MarkdownHover(
                this,
                anchor.range,
                [( (new MarkdownString())).appendText(( localize(5162, "Loading...")))],
                false,
                2000
            ))
        );
...
class MarkdownRenderedHoverParts {
    ...
    _renderHoverPart(hoverPart, onFinishedRendering) {
        ...
        if (!hoverSource) {
            return (
                 (new RenderedMarkdownHoverPart(hoverPart, renderedMarkdownElement, disposables))
            );
        }
    ...
    _renderHoverExpansionAction(container, action, actionEnabled) {
        ...
        const hoverDelegate = ( (new WorkbenchHoverDelegate(
            'mouse',
            false,
            { target: container, position: { hoverPosition: HoverPosition.LEFT } },
            this._configurationService,
            this._hoverService
        )));
...
function labelForHoverVerbosityAction(keybindingService, action) {
    switch (action) {
        case HoverVerbosityAction.Increase: {
            ...
            return kb ?
                ( localize(5165, "Increase Hover Verbosity ({0})", kb.getLabel())) :
                ( localize(5166, "Increase Hover Verbosity"));
        }
        case HoverVerbosityAction.Decrease: {
            const kb = keybindingService.lookupKeybinding(DECREASE_HOVER_VERBOSITY_ACTION_ID);
            return kb ?
                ( localize(5167, "Decrease Hover Verbosity ({0})", kb.getLabel())) :
                ( localize(5168, "Decrease Hover Verbosity"));
        }
    }
}
