...
let LinkDetector = class LinkDetector extends Disposable {
    ...
    openLinkOccurrence(occurrence, openToSide, fromUserGesture = false) {
        ...
        link.resolve(CancellationToken.None).then(uri => {
            ...
            if (messageOrError === 'invalid') {
                ...
                this.notificationService.warn(( localize(1509, 'Failed to open this link because its target is missing.')));
            }
            else {
                onUnexpectedError(err);
            }
    ...
    isEnabled(mouseEvent, withKey) {
        return Boolean((mouseEvent.target.type === MouseTargetType.CONTENT_TEXT)
...
function getHoverMessage(link, useMetaKey) {
    ...
    const label = link.tooltip
        ? link.tooltip
        : executeCmd
            ? ( localize(1510, 'Execute command'))
            : ( localize(1511, 'Follow link'));
    const kb = useMetaKey
        ? isMacintosh
            ? ( localize(1512, "cmd + click"))
            : ( localize(1513, "ctrl + click"))
        : isMacintosh
            ? ( localize(1514, "option + click"))
            : ( localize(1515, "alt + click"));
    if (link.url) {
        let nativeLabel = '';
        if (/^command:/i.test(( (link.url.toString())))) {
            ...
            if (match) {
                ...
                nativeLabel = ( localize(1516, "Execute command {0}", commandId));
            }
        }
