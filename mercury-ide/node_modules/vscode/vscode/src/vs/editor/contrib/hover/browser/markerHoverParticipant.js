...
import { append, addDisposableListener, getDomNodePagePosition, $ as $$1 } from '../../../../base/browser/dom.js';
...
import { quickFixCommandId, getCodeActions } from '../../codeAction/browser/codeAction.js';
...
import { MarkerSeverity, IMarkerData } from '../../../../platform/markers/common/markers.js';
...
let MarkerHoverParticipant = class MarkerHoverParticipant {
    ...
    renderHoverParts(context, hoverParts) {
        ...
        return (
             (new RenderedHoverParts(renderedHoverParts))
        );
    }
    ...
    _renderMarkerHover(markerHover) {
        ...
        if (isNonEmptyArray(relatedInformation)) {
            for (const { message, resource, startLineNumber, startColumn } of relatedInformation) {
                ...
                const messageElement = append(relatedInfoContainer, $('span'));
                messageElement.innerText = message;
    ...
    renderMarkerStatusbar(context, markerHover, disposables) {
        if (markerHover.marker.severity === MarkerSeverity.Error || markerHover.marker.severity === MarkerSeverity.Warning || markerHover.marker.severity === MarkerSeverity.Info) {
            ...
            if (markerController) {
                context.statusBar.addAction({
                    label: ( localize(5169, "View Problem")),
        ...
        if (!this._editor.getOption(EditorOption.readOnly)) {
            ...
            if (this.recentMarkerCodeActionsInfo) {
                if (IMarkerData.makeKey(this.recentMarkerCodeActionsInfo.marker) === IMarkerData.makeKey(markerHover.marker)) {
                    if (!this.recentMarkerCodeActionsInfo.hasCodeActions) {
                        quickfixPlaceholderElement.textContent = ( localize(5170, "No quick fixes available"));
                    }
                }
                else {
                    this.recentMarkerCodeActionsInfo = undefined;
            ...
            const updatePlaceholderDisposable = this.recentMarkerCodeActionsInfo && !this.recentMarkerCodeActionsInfo.hasCodeActions ? Disposable.None : disposableTimeout(() => quickfixPlaceholderElement.textContent = ( localize(5171, "Checking for quick fixes...")), 200, disposables);
    ...
    getCodeActions(marker) {
        return createCancelablePromise(cancellationToken => {
            return getCodeActions(this._languageFeaturesService.codeActionProvider, this._editor.getModel(), ( (new Range(
                marker.startLineNumber,
                marker.startColumn,
                marker.endLineNumber,
                marker.endColumn
            ))), markerCodeActionTrigger, Progress.None, cancellationToken);
        });
    }
};
...
export { MarkerHover, MarkerHoverParticipant };
