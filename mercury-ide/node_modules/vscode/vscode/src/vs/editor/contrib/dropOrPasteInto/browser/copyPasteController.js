...
import { createCancelablePromise, raceCancellation, DeferredPromise } from '../../../../base/common/async.js';
...
const pasteWidgetVisibleCtx = ( (new RawContextKey('pasteWidgetVisible', false, ( localize(4993, "Whether the paste widget is showing")))));
...
let CopyPasteController = class CopyPasteController extends Disposable {
    ...
    constructor(editor, instantiationService, _bulkEditService, _clipboardService, _languageFeaturesService, _quickInputService, _progressService) {
        ...
        this._postPasteWidgetManager = this._register(instantiationService.createInstance(PostEditWidgetManager, 'pasteIntoEditor', editor, pasteWidgetVisibleCtx, { id: changePasteTypeCommandId, label: ( localize(4994, "Show paste options...")) }));
    }
    changePasteType() {
    ...
    doPasteInline(allProviders, selections, dataTransfer, metadata, clipboardEvent) {
                             ...
                             (new Promise((resolve, reject) => {
                                (async () => {
                                    try {
                                        ...
                                        const resolved = resolveP && (await this._pasteProgressManager.showWhile(selections[0].getEndPosition(), ( localize(4996, "Resolving paste edit. Click to cancel")), Promise.race([showP.p, resolveP]), {
                                            cancel: () => {
                                                showP.cancel();
        ...
        this._pasteProgressManager.showWhile(selections[0].getEndPosition(), ( localize(4997, "Running paste handlers. Click to cancel and do basic paste")), p, {
            cancel: async () => {
                try {
                    p.cancel();
    ...
    showPasteAsPick(preference, allProviders, selections, dataTransfer, metadata) {
        const p = createCancelablePromise(async (token) => {
            ...
            const model = editor.getModel();
            const disposables = ( (new DisposableStore()));
            const tokenSource = disposables.add(( (new EditorStateCancellationTokenSource(
                editor,
                CodeEditorStateFlag.Value | CodeEditorStateFlag.Selection,
                undefined,
                token
            ...
            try {
                ...
                if (preference) {
                    ...
                    const selected = await this._quickInputService.pick(( (editSession.edits.map((edit) => ({
                        ...
                        placeHolder: ( localize(4998, "Select Paste Action")),
                    });
        ...
        this._progressService.withProgress({
            ...
            title: ( localize(4999, "Running paste handlers")),
        }, () => p);
    }
    ...
    providerMatchesPreference(provider, preference) {
        if (preference instanceof HierarchicalKind) {
            ...
            return (
                 (provider.providedPasteEditKinds.some(providedKind => preference.contains(providedKind)))
            );
        }
        else {
