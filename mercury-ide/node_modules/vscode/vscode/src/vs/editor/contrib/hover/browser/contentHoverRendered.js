...
class RenderedContentHoverParts extends Disposable {
    ...
    _renderStatusBar(fragment, statusBar) {
        ...
        return (
             (new RenderedStatusBar(fragment, statusBar))
        );
    }
    ...
    getAccessibleHoverContentAtIndex(index) {
        ...
        if (renderedPart.type === 'statusBar') {
            const statusBarDescription = [( localize(11233, "This is a hover status bar."))];
            for (const action of renderedPart.actions) {
                ...
                if (keybinding) {
                    ...
                    statusBarDescription.push(( localize(11235, "It has an action with label {0}.", action.actionLabel)));
                }
            }
