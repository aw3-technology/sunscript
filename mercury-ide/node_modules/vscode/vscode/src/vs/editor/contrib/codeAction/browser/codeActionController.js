...
import { CodeActionTriggerSource, CodeActionAutoApply, CodeActionKind } from '../common/types.js';
...
let CodeActionController = class CodeActionController extends Disposable {
    ...
    constructor(editor, markerService, contextKeyService, instantiationService, languageFeaturesService, progressService, _commandService, _configurationService, _actionWidgetService, _instantiationService) {
        ...
        this._editor = editor;
        this._model = this._register(( (new CodeActionModel(
            this._editor,
                ...
                if (action && action.action) {
                    ...
                    if (ranges && ranges.length > 0) {
                        const decorations = (diagnostics && diagnostics?.length > 1)
                            ? ( (diagnostics.map(
                            diagnostic => ({ range: diagnostic, options: CodeActionController_1.DECORATION })
                        )))
                            : ( (ranges.map(range => ({ range, options: CodeActionController_1.DECORATION }))));
                        currentDecorations.set(decorations);
                    }
                    else if (diagnostics && diagnostics.length > 0) {
                        const decorations = ( (diagnostics.map(
                            diagnostic => ({ range: diagnostic, options: CodeActionController_1.DECORATION })
                        )));
                        currentDecorations.set(decorations);
    ...
    _getActionBarActions(actions, at, options) {
        ...
        if (options.includeDisabledActions && actions.validActions.length > 0 && actions.allActions.length !== actions.validActions.length) {
            resultActions.push(this._showDisabled ? {
                id: 'hideMoreActions',
                label: ( localize(5029, 'Hide Disabled')),
                enabled: true,
                tooltip: '',
                class: undefined,
                run: () => {
                ...
                id: 'showMoreActions',
                label: ( localize(5030, 'Show Disabled')),
                enabled: true,
                tooltip: '',
                class: undefined,
                run: () => {
