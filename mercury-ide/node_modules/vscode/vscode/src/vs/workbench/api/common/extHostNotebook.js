...
import { FileOperationError, FileOperationResult, FileType, FilePermission, etag, ETAG_DISABLED } from '../../../platform/files/common/files.js';
...
class ExtHostNotebookController {
    ...
    async $saveNotebook(handle, uriComponents, versionId, options, token) {
        ...
        if (!this._extHostFileSystem.value.isWritableFileSystem(uri.scheme)) {
            throw new FileOperationError(localize(4585, "Unable to modify read-only file '{0}'", this._resourceForError(uri)), FileOperationResult.FILE_PERMISSION_DENIED);
        }
        const data = {
    ...
    async _validateWriteFile(uri, options) {
        ...
        if (typeof options?.mtime === 'number' && typeof options.etag === 'string' && options.etag !== ETAG_DISABLED &&
            ...
            throw new FileOperationError(localize(4586, "File Modified Since"), FileOperationResult.FILE_MODIFIED_SINCE, options);
        }
        return;
    }
