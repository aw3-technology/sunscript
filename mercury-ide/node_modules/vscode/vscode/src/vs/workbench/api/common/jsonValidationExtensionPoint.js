...
const configurationExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: ( localize(671, 'Contributes json schema configuration.')),
        type: 'array',
        ...
        configurationExtPoint.setHandler((extensions) => {
            for (const extension of extensions) {
                ...
                if (!extensionValue || !Array.isArray(extensionValue)) {
                    collector.error(( localize(674, "'configuration.jsonValidation' must be a array")));
                    return;
                }
                extensionValue.forEach(extension => {
                    ...
                    if (!isString(uri)) {
                        collector.error(( localize(676, "'configuration.jsonValidation.url' must be a URL or relative path")));
                        return;
                    }
                    if (uri.startsWith('./')) {
                        try {
...
( (Registry.as(Extensions.ExtensionFeaturesRegistry))).registerExtensionFeature({
    id: 'jsonValidation',
    label: ( localize(682, "JSON Validation")),
    access: {
        canToggle: false
    },
