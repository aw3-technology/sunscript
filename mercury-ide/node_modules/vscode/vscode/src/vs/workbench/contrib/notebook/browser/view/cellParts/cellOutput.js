...
import { RenderOutputType, JUPYTER_EXTENSION_ID } from '../../notebookBrowser.js';
...
let CellOutputElement = class CellOutputElement extends Disposable {
    ...
    _renderMissingRenderer(viewModel, preferredMimeType) {
        if (!viewModel.model.outputs.length) {
            return this._renderMessage(viewModel, ( localize(11521, "Cell has no output")));
        }
    ...
    async _attachToolbar(outputItemDiv, notebookTextModel, kernel, index, mimeTypes) {
        ...
        const pickAction = this.toolbarDisposables.add(( (new Action('notebook.output.pickMimetype', ( localize(11523, "Change Presentation")), ThemeIcon.asClassName(mimetypeIcon), undefined, async (_context) => this._pickActiveMimeTypeRenderer(outputItemDiv, notebookTextModel, kernel, this.output)))));
    ...
    async _pickActiveMimeTypeRenderer(outputItemDiv, notebookTextModel, kernel, viewModel) {
        ...
        mimeTypes.forEach((mimeType, index) => {
            if (mimeType.isTrusted) {
                ...
                arr.push({
                    ...
                    description: index === currIndex ? ( localize(11524, "Currently Active")) : undefined
                });
            }
        });
        if (( (unsupportedItems.some(m => JUPYTER_RENDERER_MIMETYPES.includes(m.id))))) {
            unsupportedItems.push({
                label: ( localize(11525, "Install additional renderers from the marketplace")),
                id: 'installRenderers',
        ...
        picker.placeholder = items.length !== mimeTypes.length
            ? ( localize(11526, "Select mimetype to render for current output"))
            : ( localize(11527, "Select mimetype to render for current output"));
    ...
    _generateRendererInfo(renderId) {
        ...
        return ( localize(11528, "renderer not available"));
    }
...
let CellOutputContainer = class CellOutputContainer extends CellContentPart {
    ...
    _updateOutputs(splice, context = CellOutputUpdateContext.Other) {
