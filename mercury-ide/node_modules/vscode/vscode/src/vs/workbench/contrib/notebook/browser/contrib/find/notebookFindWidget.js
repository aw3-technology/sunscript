...
import { append, addDisposableListener, getWindow, isHTMLElement, EventType } from '../../../../../../base/browser/dom.js';
...
import { NLS_NO_RESULTS, NLS_MATCHES_LOCATION } from '../../../../../../editor/contrib/find/browser/findWidget.js';
...
let NotebookFindWidget = class NotebookFindWidget extends SimpleFindReplaceWidget {
    ...
    _onFindInputKeyDown(e) {
        ...
        else if (e.equals(KeyMod$1.Shift | KeyCode.Enter)) {
            this.find(true);
            e.preventDefault();
            return;
        }
    ...
    _getAriaLabel(label, currentMatch, searchString) {
        if (label === NLS_NO_RESULTS) {
            return searchString === ''
                ? ( localize(10104, "{0} found", label))
                : ( localize(10105, "{0} found for '{1}'", label, searchString));
        }
        return ( localize(10106, "{0} found for '{1}'", label, searchString));
    }
    dispose() {
        this._notebookEditor?.removeClassName(FIND_SHOW_TRANSITION);
        this._notebookEditor?.removeClassName(FIND_HIDE_TRANSITION);
