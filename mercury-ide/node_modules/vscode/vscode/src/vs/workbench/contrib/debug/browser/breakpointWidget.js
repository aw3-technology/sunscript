...
import { CONTEXT_BREAKPOINT_WIDGET_VISIBLE, CONTEXT_IN_BREAKPOINT_WIDGET, BREAKPOINT_EDITOR_CONTRIBUTION_ID, BreakpointWidgetContext, DEBUG_SCHEME } from '../common/debug.js';
...
let BreakpointWidget = class BreakpointWidget extends ZoneWidget {
    ...
    _fillContainer(container) {
        ...
        const selectBox = ( (new SelectBox([
            { text: ( localize(10226, "Expression")) },
            { text: ( localize(10227, "Hit Count")) },
            { text: ( localize(10228, "Log Message")) },
            { text: ( localize(10229, "Wait for Breakpoint")) },
        ], this.context, this.contextViewService, defaultSelectBoxStyles, { ariaLabel: ( localize(10230, 'Breakpoint Type')) })));
    ...
    createModesInput(container) {
        ...
        const sb = this.selectModeBox = ( (new SelectBox([
            { text: ( localize(10231, 'Mode')), isDisabled: true },
    ...
    createTriggerBreakpointInput(container) {
        ...
        const breakpointOptions = [
            { text: ( localize(10232, 'None')), isDisabled: true },
            ...( (breakpoints.map(bp => ({
                text: `${this.labelService.getUriLabel(bp.uri, { relative: true })}: ${bp.lineNumber}`,
                description: ( localize(10233, 'Loading...'))
            })))),
        ];
        ...
        for (const [i, bp] of breakpoints.entries()) {
            this.textModelService.createModelReference(bp.uri).then(ref => {
                ...
                breakpointOptions[i + 1].description = ( localize(10234, 'Could not load source.'));
            });
        }
        const selectBreakpointBox = this.selectBreakpointBox = ( (new SelectBox(
            ...
            { ariaLabel: ( localize(10235, 'Select breakpoint')) }
        )));
        ...
        this.toDispose.push(addDisposableListener(this.selectBreakpointContainer, EventType.KEY_DOWN, e => {
        ...
        closeButton.label = ( localize(10236, "Ok"));
