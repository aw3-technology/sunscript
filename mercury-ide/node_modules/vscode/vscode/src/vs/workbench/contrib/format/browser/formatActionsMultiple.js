...
import { FormattingConflicts, FormattingMode, getRealAndSyntheticDocumentFormattersOrdered, FormattingKind, formatDocumentWithProvider, formatDocumentRangesWithProvider } from '../../../../editor/contrib/format/browser/format.js';
...
let DefaultFormatter = class DefaultFormatter extends Disposable {
    ...
    async _updateConfigValues() {
        ...
        DefaultFormatter_1.extensionItemLabels.push(( localize(4529, 'None')));
        DefaultFormatter_1.extensionDescriptions.push(( localize(4530, "None")));
    ...
    async _selectFormatter(formatter, document, mode, kind) {
        ...
        if (mode !== FormattingMode.Silent) {
            const { confirmed } = await this._dialogService.confirm({
                message: ( localize(4535, "Configure Default Formatter")),
                detail: formatterOrMessage,
                primaryButton: ( localize(4536, "&&Configure..."))
            });
            if (confirmed) {
            ...
            this._notificationService.prompt(Severity$1.Info, formatterOrMessage, [{ label: ( localize(4537, "Configure...")), run: () => this._pickAndPersistDefaultFormatter(formatter, document) }], { priority: NotificationPriority.SILENT });
        }
        return undefined;
    }
    ...
    _updateStatus() {
        ...
        this._analyzeFormatter(FormattingKind.File, formatter, document).then(result => {
            ...
            const command = { id: `formatter/configure/dfl/${generateUuid()}`, title: ( localize(4539, "Configure...")) };
...
( (Registry.as(Extensions.Workbench))).registerWorkbenchContribution(DefaultFormatter, LifecyclePhase.Restored);
...
async function showFormatterPick(accessor, model, formatters) {
    ...
    const picks = ( (formatters.map((provider, index) => {
        ...
        const pick = {
            ...
            description: isDefault ? ( localize(4543, "(default)")) : undefined,
        };
        if (isDefault) {
    ...
    const configurePick = {
        label: ( localize(4544, "Configure Default Formatter..."))
    };
    const pick = await quickPickService.pick([...picks, { type: 'separator' }, configurePick], {
        placeHolder: ( localize(4545, "Select a formatter")),
...
registerEditorAction(class FormatDocumentMultipleAction extends EditorAction {
    constructor() {
        super({
            ...
            label: ( localize(4546, "Format Document With...")),
...
registerEditorAction(class FormatSelectionMultipleAction extends EditorAction {
    constructor() {
        super({
            ...
            label: ( localize(4547, "Format Selection With...")),
            alias: 'Format Code...',
            precondition: ( (ContextKeyExpr.and(
