...
const ChatEditorIcon = registerIcon('chat-editor-label-icon', Codicon.commentDiscussion, ( localize(7137, 'Icon of the chat editor label.')));
let ChatEditorInput = class ChatEditorInput extends EditorInput {
    ...
    getName() {
        return this.model?.title || ( localize(7138, "Chat")) + (this.inputCount > 0 ? ` ${this.inputCount + 1}` : '');
    }
    getIcon() {
...
( ((function(ChatUri) {
    ...
    function generate(handle) {
        return (
             (URI.from({ scheme: ChatUri.scheme, path: `chat-${handle}` }))
        );
    }
