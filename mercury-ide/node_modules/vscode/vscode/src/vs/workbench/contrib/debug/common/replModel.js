    ...
    getChildren() {
        ...
        if (Array.isArray(this.valueObj)) {
            result = ( (this.valueObj.slice(0, RawObjectReplElement.MAX_CHILDREN)
                .map(
                (v, index) => ( (new RawObjectReplElement(`${this.id}:${index}`, String(index), v)))
            )));
        }
        else if (isObject(this.valueObj)) {
            result = ( (Object.getOwnPropertyNames(this.valueObj).slice(0, RawObjectReplElement.MAX_CHILDREN)
                .map(
                (key, index) => ( (new RawObjectReplElement(`${this.id}:${index}`, key, this.valueObj[key])))
            )));
        }
...
class ReplModel {
    ...
    appendToRepl(session, { output, expression, sev, source }) {
        ...
        if (clearAnsiIndex !== -1) {
            ...
            this.appendToRepl(session, { output: ( localize(10222, "Console was cleared")), sev: Severity$1.Ignore });
