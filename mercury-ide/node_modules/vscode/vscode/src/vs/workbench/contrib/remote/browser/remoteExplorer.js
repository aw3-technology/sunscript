...
let ForwardedPortsView = class ForwardedPortsView extends Disposable {
        ...
        if (this.remoteExplorerService.tunnelModel.forwarded.size > 0) {
            this.activityBadge.value = this.activityService.showViewActivity(TUNNEL_VIEW_ID, {
                badge: ( (new NumberBadge(
                    ...
                    n => n === 1 ? ( localize(8728, "1 forwarded port")) : ( localize(8729, "{0} forwarded ports", n))
                )))
            });
        }
    }
    ...
    get entry() {
        ...
        const text = `${count}`;
        if (count === 0) {
            tooltip = ( localize(8730, "No Ports Forwarded"));
        }
        else {
        ...
        return {
            name: ( localize(8732, "Forwarded Ports")),
            ...
            this.portListener = this._register(this.remoteExplorerService.tunnelModel.onForwardPort(async () => {
                ...
                if (Array.from(( (this.remoteExplorerService.tunnelModel.forwarded.values()))).filter(tunnel => tunnel.source.source === TunnelSource.Auto).length > fallbackAt) {
                    ...
                    this.notificationService.notify({
                        ...
                        actions: {
                            primary: [
                                ( (new Action('switchBack', ( localize(8734, "Undo")), undefined, true, async () => {
                                ...
                                ( (new Action('showPortSourceSetting', ( localize(8735, "Show Setting")), undefined, true, async () => {
...
class OnAutoForwardedAction extends Disposable {
    ...
    linkMessage() {
        return ( localize(8737, "[See all forwarded ports]({0})", `command:${TunnelPanel.ID}.focus`));
    }
    ...
    makePublicChoice(tunnel) {
        return {
            label: ( localize(8739, "Make Public")),
            run: async () => {
    ...
    elevateChoice(tunnel) {
        return {
            label: ( localize(8740, "Use Port {0} as Sudo...", tunnel.tunnelRemotePort)),
            run: async () => {
                await this.remoteExplorerService.close({ host: tunnel.tunnelRemoteHost, port: tunnel.tunnelRemotePort }, TunnelCloseReason.Other);
