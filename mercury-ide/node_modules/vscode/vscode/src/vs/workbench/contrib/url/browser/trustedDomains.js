...
const manageTrustedDomainSettingsCommand = {
    ...
    description: {
        description: ( localize2(4604, 'Manage Trusted Domains')),
        args: []
    },
...
async function configureOpenerTrustedDomainsHandler(trustedDomains, domainToConfigure, resource, quickInputService, storageService, editorService, telemetryService) {
    ...
    options.push({
        type: 'item',
        label: ( localize(4605, 'Trust {0}', domainToConfigure)),
        id: 'trust',
    ...
    if (isIP) {
        if (parsedDomainToConfigure.authority.includes(':')) {
            ...
            options.push({
                type: 'item',
                label: ( localize(4606, 'Trust {0} on all ports', base)),
                toTrust: base + ':*',
                id: 'trust'
            });
        }
        ...
        options.push({
            type: 'item',
            label: ( localize(4607, 'Trust {0} and all its subdomains', domainEnd)),
    ...
    options.push({
        type: 'item',
        label: ( localize(4608, 'Trust all domains (disables link protection)')),
        toTrust: '*',
        id: 'trust'
    });
    options.push({
        type: 'item',
        label: ( localize(4609, 'Manage Trusted Domains')),
        id: 'manage'
    });
