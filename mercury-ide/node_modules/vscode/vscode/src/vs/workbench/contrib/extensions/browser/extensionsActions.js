...
import { ExtensionState, ExtensionEditorTab, AutoUpdateConfigurationKey, INSTALL_ACTIONS_GROUP, UPDATE_ACTIONS_GROUP, THEME_ACTIONS_GROUP, ExtensionRuntimeActionType, VIEWLET_ID, TOGGLE_IGNORE_EXTENSION_ACTION_ID, SELECT_INSTALL_VSIX_EXTENSION_COMMAND_ID } from '../common/extensions.js';
...
import { InstallOperation, ExtensionManagementErrorCode, TargetPlatformToString } from '../../../../platform/extensionManagement/common/extensionManagement.js';
import { IExtensionGalleryService } from '../../../../platform/extensionManagement/common/extensionManagement.service.js';
import { EnablementState } from '../../../services/extensionManagement/common/extensionManagement.js';
import { IWorkbenchExtensionEnablementService, IExtensionManagementServerService, IWorkbenchExtensionManagementService } from '../../../services/extensionManagement/common/extensionManagement.service.js';
...
import { ExtensionType, isLanguagePackExtension, ExtensionIdentifier, TargetPlatform, getWorkspaceSupportTypeMessage, isApplicationScopedExtension } from '../../../../platform/extensions/common/extensions.js';
...
import { language, isWeb, isIOS } from '../../../../base/common/platform.js';
...
let PromptExtensionInstallFailureAction = class PromptExtensionInstallFailureAction extends Action {
    ...
    async run() {
        ...
        if (this.error.name === ExtensionManagementErrorCode.Unsupported) {
            const productName = isWeb ? ( localize(7874, "{0} for the Web", this.productService.nameLong)) : this.productService.nameLong;
            ...
            const { confirmed } = await this.dialogService.confirm({
                ...
                message,
                primaryButton: ( localize(7876, "&&More Information")),
                cancelButton: ( localize(7877, "Close"))
            });
            if (confirmed) {
        ...
        if (ExtensionManagementErrorCode.ReleaseVersionNotFound === this.error.name) {
            await this.dialogService.prompt({
                ...
                buttons: [{
                        label: ( localize(7878, "Install Pre-Release")),
                        run: () => {
                ...
                cancelButton: ( localize(7879, "Cancel"))
            });
            return;
        }
        ...
        if (ExtensionManagementErrorCode.Signature === this.error.name) {
            await this.dialogService.prompt({
                ...
                buttons: [{
                        label: ( localize(7881, "Install Anyway")),
                        run: () => {
        ...
        if (downloadUrl) {
            ...
            promptChoices.push({
                label: ( localize(7885, "Try Downloading Manually...")),
                run: () => this.openerService.open(downloadUrl).then(() => {
                    this.notificationService.prompt(Severity$1.Info, ( localize(
                        ...
                        this.extension.identifier.id
                            ...
                            label: ( localize(7887, "Install from VSIX...")),
...
let InstallAction = class InstallAction extends ExtensionAction {
    ...
    constructor(options, extensionsWorkbenchService, instantiationService, runtimeExtensionService, workbenchThemeService, labelService, dialogService, preferencesService, telemetryService, contextService) {
        super('extensions.install', ( localize(7888, "Install")), InstallAction_1.CLASS, false);
    ...
    async run() {
        ...
        if (this.extension.deprecationInfo) {
            let detail = ( localize(7889, "This extension is deprecated as it is no longer being maintained."));
            ...
            if (this.extension.deprecationInfo.extension) {
                ...
                buttons.push({
                    label: ( localize(7891, "&&Open {0}", this.extension.deprecationInfo.extension.displayName)),
                    run: async () => {
                ...
                buttons.push({
                    label: ( localize(7893, "&&Configure Settings")),
                    run: async () => {
        ...
        if (extension?.local) {
            alert(( localize(7896, "Installing extension {0} is completed.", this.extension.displayName)));
    ...
    getLabel(primary) {
        if (this.extension?.isWorkspaceScoped && this.extension.resourceExtension && this.contextService.isInsideWorkspace(this.extension.resourceExtension.location)) {
            return ( localize(7897, "Install Workspace Extension"));
        }
        if (this.options.installPreReleaseVersion && this.extension?.hasPreReleaseVersion) {
            return primary ? ( localize(7898, "Install Pre-Release")) : ( localize(7899, "Install Pre-Release Version"));
        }
        if (this.extension?.hasPreReleaseVersion) {
            return primary ? ( localize(7888, "Install")) : ( localize(7900, "Install Release Version"));
        }
        return ( localize(7888, "Install"));
    }
};
...
class InstallingLabelAction extends ExtensionAction {
    static { this.LABEL = ( localize(7901, "Installing")); }
...
let InstallInOtherServerAction = class InstallInOtherServerAction extends ExtensionAction {
    ...
    static { this.INSTALL_LABEL = ( localize(7888, "Install")); }
    static { this.INSTALLING_LABEL = ( localize(7901, "Installing")); }
...
let LocalInstallAction = class LocalInstallAction extends InstallInOtherServerAction {
    ...
    getInstallLabel() {
        return ( localize(7903, "Install Locally"));
    }
};
...
let WebInstallAction = class WebInstallAction extends InstallInOtherServerAction {
    ...
    getInstallLabel() {
        return ( localize(7904, "Install in Browser"));
    }
};
...
let UninstallAction = class UninstallAction extends ExtensionAction {
    ...
    static { this.UninstallLabel = ( localize(7905, "Uninstall")); }
    static { this.UninstallingLabel = ( localize(7906, "Uninstalling")); }
    ...
    async run() {
        ...
        alert(( localize(7907, "Uninstalling extension {0} started.", this.extension.displayName)));
        try {
...
let UpdateAction = class UpdateAction extends ExtensionAction {
    ...
    constructor(verbose, extensionsWorkbenchService, dialogService, openerService, instantiationService) {
        super(`extensions.update`, ( localize(7909, "Update")), UpdateAction_1.DisabledClass, false);
    ...
    update() {
        this.updateThrottler.queue(() => this.computeAndUpdateEnablement());
        if (this.extension) {
            this.label = this.verbose ? ( localize(7910, "Update to v{0}", this.extension.latestVersion)) : ( localize(7909, "Update"));
        }
    }
    async computeAndUpdateEnablement() {
        this.enabled = false;
    ...
    async run() {
        ...
        if (consent) {
            const { result } = await this.dialogService.prompt({
                type: 'warning',
                title: ( localize(7911, "Update {0} Extension", this.extension.displayName)),
                message: ( localize(7912, "{0}\n\nWould you like to update the extension?", consent)),
                buttons: [{
                        label: ( localize(7909, "Update")),
                        run: () => 'update'
                    }, {
                        label: ( localize(7913, "Review")),
                        run: () => 'review'
                    }, {
                        label: ( localize(7879, "Cancel")),
                        run: () => 'cancel'
                    }]
            });
            if (result === 'cancel') {
...
let ToggleAutoUpdateForExtensionAction = class ToggleAutoUpdateForExtensionAction extends ExtensionAction {
    ...
    static { this.LABEL = ( localize2(7916, "Auto Update")); }
    ...
    async run() {
        ...
        if (enableAutoUpdate) {
            alert(( localize(7917, "Enabled auto updates for", this.extension.displayName)));
        }
        else {
            alert(( localize(7918, "Disabled auto updates for", this.extension.displayName)));
        }
    }
};
...
let ToggleAutoUpdatesForPublisherAction = class ToggleAutoUpdatesForPublisherAction extends ExtensionAction {
    ...
    static { this.LABEL = ( localize(7919, "Auto Update All (From Publisher)")); }
...
let MigrateDeprecatedExtensionAction = class MigrateDeprecatedExtensionAction extends ExtensionAction {
    ...
    constructor(small, extensionsWorkbenchService) {
        super('extensionsAction.migrateDeprecatedExtension', ( localize(7921, "Migrate")), MigrateDeprecatedExtensionAction_1.DisabledClass, false);
        this.small = small;
        this.extensionsWorkbenchService = extensionsWorkbenchService;
        this.update();
    }
    update() {
        ...
        this.label = this.small ? ( localize(7923, "Migrate")) : this.tooltip;
    }
    async run() {
        if (!this.extension?.deprecationInfo?.extension) {
            return;
...
let ManageExtensionAction = class ManageExtensionAction extends DropDownExtensionAction {
    ...
    constructor(instantiationService, extensionService, contextKeyService) {
        ...
        this.tooltip = ( localize(7924, "Manage"));
        this.update();
    }
...
let TogglePreReleaseExtensionAction = class TogglePreReleaseExtensionAction extends ExtensionAction {
    ...
    static { this.LABEL = ( localize(7925, "Pre-Release")); }
    ...
    update() {
        ...
        if (this.extension.preRelease) {
            this.label = ( localize(7926, "Switch to Release Version"));
            this.tooltip = ( localize(7927, "This will switch and enable updates to release versions"));
        }
        else {
            this.label = ( localize(7928, "Switch to Pre-Release Version"));
            this.tooltip = ( localize(
                7929,
...
let InstallAnotherVersionAction = class InstallAnotherVersionAction extends ExtensionAction {
    ...
    static { this.LABEL = ( localize(7930, "Install Specific Version...")); }
    ...
    async run() {
        ...
        if (!allVersions.length) {
            await this.dialogService.info(( localize(7931, "This extension has no other versions.")));
            return;
        }
        const picks = ( (allVersions.map((v, i) => {
            return {
                id: v.version,
                label: v.version,
                description: `${fromNow(( (new Date(Date.parse(v.date)))), true)}${v.isPreReleaseVersion ? ` (${( localize(7932, "pre-release"))})` : ''}${v.version === this.extension?.local?.manifest.version ? ` (${( localize(7933, "current"))})` : ''}`,
                ariaLabel: `${v.isPreReleaseVersion ? 'Pre-Release version' : 'Release version'} ${v.version}`,
                isPreReleaseVersion: v.isPreReleaseVersion
            };
        })));
        const pick = await this.quickInputService.pick(picks, {
            placeHolder: ( localize(7934, "Select Version to Install")),
            matchOnDetail: true
        });
        if (pick) {
...
let EnableForWorkspaceAction = class EnableForWorkspaceAction extends ExtensionAction {
    ...
    static { this.LABEL = ( localize(7935, "Enable (Workspace)")); }
    constructor(extensionsWorkbenchService, extensionEnablementService) {
        ...
        this.extensionsWorkbenchService = extensionsWorkbenchService;
        this.extensionEnablementService = extensionEnablementService;
        this.tooltip = ( localize(7936, "Enable this extension only in this workspace"));
        this.update();
    }
    update() {
        this.enabled = false;
...
let DisableForWorkspaceAction = class DisableForWorkspaceAction extends ExtensionAction {
    ...
    static { this.LABEL = ( localize(7939, "Disable (Workspace)")); }
    constructor(workspaceContextService, extensionsWorkbenchService, extensionEnablementService, extensionService) {
        ...
        this.extensionEnablementService = extensionEnablementService;
        this.extensionService = extensionService;
        this.tooltip = ( localize(7940, "Disable this extension only in this workspace"));
        this.update();
...
let DisableGloballyAction = class DisableGloballyAction extends ExtensionAction {
    ...
    static { this.LABEL = ( localize(7941, "Disable")); }
    constructor(extensionsWorkbenchService, extensionEnablementService, extensionService) {
        ...
        this.extensionEnablementService = extensionEnablementService;
        this.extensionService = extensionService;
        this.tooltip = ( localize(7942, "Disable this extension"));
        this.update();
        this._register(this.extensionService.onDidChangeExtensions(() => this.update()));
    }
    update() {
...
let ExtensionRuntimeStateAction = class ExtensionRuntimeStateAction extends ExtensionAction {
    ...
    update() {
        ...
        this.label = runtimeState.action === ExtensionRuntimeActionType.ReloadWindow ? ( localize(7943, 'Reload Window'))
            : runtimeState.action === ExtensionRuntimeActionType.RestartExtensions ? ( localize(7944, 'Restart Extensions'))
                : runtimeState.action === ExtensionRuntimeActionType.QuitAndInstall ? ( localize(7945, 'Restart to Update'))
                    : runtimeState.action === ExtensionRuntimeActionType.ApplyUpdate || runtimeState.action === ExtensionRuntimeActionType.DownloadUpdate ? ( localize(7946, 'Update {0}', this.productService.nameShort)) : '';
    }
    async run() {
        const runtimeState = this.extension?.runtimeState;
...
function getQuickPickEntries(themes, currentTheme, extension, showCurrentTheme) {
    ...
    if (showCurrentTheme) {
        picks.push({ type: 'separator', label: ( localize(7933, "current")) });
...
let SetColorThemeAction = class SetColorThemeAction extends ExtensionAction {
    ...
    static { this.TITLE = ( localize2(7947, 'Set Color Theme')); }
    ...
    async run({ showCurrentTheme, ignoreFocusLost } = { showCurrentTheme: false, ignoreFocusLost: false }) {
        ...
        const pickedTheme = await this.quickInputService.pick(picks, {
            placeHolder: ( localize(7948, "Select Color Theme")),
...
let SetFileIconThemeAction = class SetFileIconThemeAction extends ExtensionAction {
    ...
    static { this.TITLE = ( localize2(7949, 'Set File Icon Theme')); }
    static { this.EnabledClass = `${ExtensionAction.LABEL_ACTION_CLASS} theme`; }
    static { this.DisabledClass = `${this.EnabledClass} disabled`; }
    constructor(extensionService, workbenchThemeService, quickInputService, extensionEnablementService) {
    ...
    async run({ showCurrentTheme, ignoreFocusLost } = { showCurrentTheme: false, ignoreFocusLost: false }) {
        ...
        const pickedTheme = await this.quickInputService.pick(picks, {
            placeHolder: ( localize(7950, "Select File Icon Theme")),
...
let SetProductIconThemeAction = class SetProductIconThemeAction extends ExtensionAction {
    ...
    static { this.TITLE = ( localize2(7951, 'Set Product Icon Theme')); }
    static { this.EnabledClass = `${ExtensionAction.LABEL_ACTION_CLASS} theme`; }
    static { this.DisabledClass = `${this.EnabledClass} disabled`; }
    constructor(extensionService, workbenchThemeService, quickInputService, extensionEnablementService) {
    ...
    async run({ showCurrentTheme, ignoreFocusLost } = { showCurrentTheme: false, ignoreFocusLost: false }) {
        ...
        const pickedTheme = await this.quickInputService.pick(picks, {
            placeHolder: ( localize(7952, "Select Product Icon Theme")),
...
let SetLanguageAction = class SetLanguageAction extends ExtensionAction {
    ...
    static { this.TITLE = ( localize2(7953, 'Set Display Language')); }
...
let ClearLanguageAction = class ClearLanguageAction extends ExtensionAction {
    ...
    static { this.TITLE = ( localize2(7954, 'Clear Display Language')); }
    static { this.EnabledClass = `${ExtensionAction.LABEL_ACTION_CLASS} language`; }
    static { this.DisabledClass = `${this.EnabledClass} disabled`; }
...
let ShowRecommendedExtensionAction = class ShowRecommendedExtensionAction extends Action {
    ...
    static { this.LABEL = ( localize(7955, "Show Recommended Extension")); }
...
let InstallRecommendedExtensionAction = class InstallRecommendedExtensionAction extends Action {
    ...
    static { this.LABEL = ( localize(7956, "Install Recommended Extension")); }
...
let IgnoreExtensionRecommendationAction = class IgnoreExtensionRecommendationAction extends Action {
    ...
    constructor(extension, extensionRecommendationsManagementService) {
        ...
        this.tooltip = ( localize(7957, "Do not recommend this extension again"));
        this.enabled = true;
    }
    run() {
...
let UndoIgnoreExtensionRecommendationAction = class UndoIgnoreExtensionRecommendationAction extends Action {
    ...
    constructor(extension, extensionRecommendationsManagementService) {
        ...
        this.tooltip = ( localize(7958, "Undo"));
        this.enabled = true;
    }
    run() {
...
let SearchExtensionsAction = class SearchExtensionsAction extends Action {
    constructor(searchValue, paneCompositeService) {
        super('extensions.searchExtensions', ( localize(7959, "Search Extensions")), undefined, true);
        this.searchValue = searchValue;
        this.paneCompositeService = paneCompositeService;
    }
    async run() {
        const viewPaneContainer = (await this.paneCompositeService.openPaneComposite(VIEWLET_ID, ViewContainerLocation.Sidebar, true))?.getViewPaneContainer();
...
let ConfigureWorkspaceRecommendedExtensionsAction = class ConfigureWorkspaceRecommendedExtensionsAction extends AbstractConfigureRecommendedExtensionsAction {
    ...
    static { this.LABEL = ( localize(7961, "Configure Recommended Extensions (Workspace)")); }
...
let ConfigureWorkspaceFolderRecommendedExtensionsAction = class ConfigureWorkspaceFolderRecommendedExtensionsAction extends AbstractConfigureRecommendedExtensionsAction {
    ...
    static { this.LABEL = ( localize(7962, "Configure Recommended Extensions (Workspace Folder)")); }
    ...
    computeLabel() {
        ...
        if (currentStatus !== null) {
            if (currentStatus === ExtensionState.Installing && this.status === ExtensionState.Installed) {
                return canAddExtension() ? this.initialStatus === ExtensionState.Installed && this.version !== currentVersion ? ( localize(7963, "Updated")) : ( localize(7964, "Installed")) : null;
            }
            if (currentStatus === ExtensionState.Uninstalling && this.status === ExtensionState.Uninstalled) {
                this.initialStatus = this.status;
                return canRemoveExtension() ? ( localize(7965, "Uninstalled")) : null;
            }
        }
        if (currentEnablementState !== null) {
            ...
            if (!currentlyEnabled && enabled) {
                return canAddExtension() ? ( localize(7966, "Enabled")) : null;
            }
            if (currentlyEnabled && !enabled) {
                return canRemoveExtension() ? ( localize(7967, "Disabled")) : null;
            }
        }
        return null;
    }
...
let ToggleSyncExtensionAction = class ToggleSyncExtensionAction extends DropDownExtensionAction {
    ...
    update() {
        ...
        if (this.extension) {
            ...
            this.tooltip = isIgnored ? ( localize(7968, "This extension is ignored during sync")) : ( localize(7969, "This extension is synced"));
        }
    }
    async run() {
        return super.run({
            actionGroups: [
                [
                    ...
                    this.extensionsWorkbenchService.isExtensionIgnoredToSync(this.extension) ? ( localize(7970, "Sync this extension")) : ( localize(7971, "Do not sync this extension")),
                    undefined,
                    true,
...
let ExtensionStatusAction = class ExtensionStatusAction extends ExtensionAction {
    ...
    async computeAndUpdateStatus() {
        ...
        if (this.extension.isMalicious) {
            this.updateStatus({ icon: warningIcon, message: ( (new MarkdownString(( localize(7972, "This extension was reported to be problematic."))))) }, true);
            return;
        }
        if (this.extension.deprecationInfo) {
            if (this.extension.deprecationInfo.extension) {
                ...
                this.updateStatus({ icon: warningIcon, message: ( (new MarkdownString(( localize(7973, "This extension is deprecated. Use the {0} extension instead.", link))))) }, true);
            }
            else if (this.extension.deprecationInfo.settings) {
                const link = `[${( localize(7974, "settings"))}](${( (URI.parse(
                ...
                const message = ( (new MarkdownString(( localize(7976, "This extension is deprecated as it is no longer being maintained.")))));
        ...
        if (this.extension.gallery && this.extension.state === ExtensionState.Uninstalled && !(await this.extensionsWorkbenchService.canInstall(this.extension))) {
            if (this.extensionManagementServerService.localExtensionManagementServer || this.extensionManagementServerService.remoteExtensionManagementServer) {
                ...
                ))} [${( localize(7979, "Learn More"))}](https://aka.ms/vscode-platform-specific-extensions)`)));
                this.updateStatus({ icon: warningIcon, message }, true);
                return;
            }
            if (this.extensionManagementServerService.webExtensionManagementServer) {
                const productName = ( localize(7874, "{0} for the Web", this.productService.nameLong));
                const message = ( (new MarkdownString(`${( localize(
                    7980,
                    "The '{0}' extension is not available in {1}.",
                    this.extension.displayName || this.extension.identifier.id,
                    productName
                ))} [${( localize(7981, "Learn Why"))}](https://aka.ms/vscode-web-extensions-guide)`)));
                this.updateStatus({ icon: warningIcon, message }, true);
                return;
            }
        }
        ...
        if (this.extension.enablementState === EnablementState.DisabledByEnvironment) {
            this.updateStatus({ message: ( (new MarkdownString(( localize(7982, "This extension is disabled by the environment."))))) }, true);
            return;
        }
                    ...
                    if (this.extensionManifestPropertiesService.prefersExecuteOnWorkspace(this.extension.local.manifest)) {
                        if (this.extensionManagementServerService.remoteExtensionManagementServer) {
                            message = ( (new MarkdownString(`${( localize(
                                ...
                                this.extensionManagementServerService.remoteExtensionManagementServer.label
                            ))} [${( localize(7979, "Learn More"))}](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`)));
                        }
                    }
                }
        ...
        for (const feature of features) {
            ...
            const manageAccessLink = `[${( localize(7992, 'Manage Access'))}](${( (URI.parse(
        ...
        if (this.extensionManagementServerService.remoteExtensionManagementServer) {
            ...
            if (this.extension.server === this.extensionManagementServerService.localExtensionManagementServer && runningExtensionServer === this.extensionManagementServerService.remoteExtensionManagementServer) {
                if (this.extensionManifestPropertiesService.prefersExecuteOnWorkspace(this.extension.local.manifest)) {
                    ...
                    ))} [${( localize(7979, "Learn More"))}](https://code.visualstudio.com/api/advanced-topics/remote-extensions#architecture-and-extension-kinds)`))) }, true);
                }
                return;
            }
        ...
        if (!this.extension.isWorkspaceScoped && isEnabled && isRunning) {
            if (this.extension.enablementState === EnablementState.EnabledWorkspace) {
                this.updateStatus({ message: ( (new MarkdownString(( localize(7999, "This extension is enabled for this workspace by the user."))))) }, true);
                return;
            }
            if (this.extensionManagementServerService.localExtensionManagementServer && this.extensionManagementServerService.remoteExtensionManagementServer) {
                if (this.extension.server === this.extensionManagementServerService.remoteExtensionManagementServer) {
                    this.updateStatus({ message: ( (new MarkdownString(( localize(8000, "Extension is enabled on '{0}'", this.extension.server.label))))) }, true);
                    return;
                }
            }
        ...
        if (!isEnabled && !isRunning) {
            if (this.extension.enablementState === EnablementState.DisabledGlobally) {
                this.updateStatus({ message: ( (new MarkdownString(( localize(8001, "This extension is disabled globally by the user."))))) }, true);
                return;
            }
            if (this.extension.enablementState === EnablementState.DisabledWorkspace) {
                this.updateStatus({ message: ( (new MarkdownString(( localize(8002, "This extension is disabled for this workspace by the user."))))) }, true);
                return;
            }
        }
...
let ReinstallAction = class ReinstallAction extends Action {
    ...
    static { this.LABEL = ( localize(8003, "Reinstall Extension...")); }
    ...
    run() {
        return this.quickInputService.pick(this.getEntries(), { placeHolder: ( localize(8004, "Select Extension to Reinstall")) })
        ...
        return this.instantiationService.createInstance(SearchExtensionsAction, '@installed ').run()
            ...
            return this.extensionsWorkbenchService.reinstall(extension)
                ...
                const actions = requireReload ? [{
                        label: ( localize(8007, "Reload Now")),
                        run: () => this.hostService.reload()
                    }] : [];
                this.notificationService.prompt(Severity$1.Info, message, actions, { sticky: true });
...
let InstallSpecificVersionOfExtensionAction = class InstallSpecificVersionOfExtensionAction extends Action {
    ...
    static { this.LABEL = ( localize(8008, "Install Specific Version of Extension...")); }
    ...
    async run() {
        const extensionPick = await this.quickInputService.pick(this.getExtensionEntries(), { placeHolder: ( localize(8009, "Select Extension")), matchOnDetail: true });
...
let AbstractInstallExtensionsInServerAction = class AbstractInstallExtensionsInServerAction extends Action {
    ...
    async selectAndInstallExtensions() {
        ...
        if (localExtensionsToInstall.length) {
            ...
            quickPick.placeholder = ( localize(8010, "Select extensions to install"));
            ...
            this.notificationService.notify({
                ...
                message: ( localize(8011, "There are no extensions to install."))
            });
        }
    }
        ...
        if (selectedItems.length) {
            ...
            if (localExtensionsToInstall.length) {
                await this.progressService.withProgress({
                    ...
                    title: ( localize(8012, "Installing Extensions..."))
                }, () => this.installExtensions(localExtensionsToInstall));
                this.notificationService.info(( localize(8013, "Successfully installed extensions.")));
            }
        }
    }
};
...
let InstallRemoteExtensionsInLocalAction = class InstallRemoteExtensionsInLocalAction extends AbstractInstallExtensionsInServerAction {
    ...
    get label() {
        return ( localize(8016, "Install Remote Extensions Locally..."));
    }
    getQuickPickTitle() {
        return ( localize(8017, "Install Remote Extensions Locally"));
    }
    getExtensionsToInstall(local) {
...
registerColor('extensionButton.background', {
    ...
    hcDark: null,
    hcLight: null
}, ( localize(8018, "Button background color for extension actions.")));
registerColor('extensionButton.foreground', {
    dark: buttonForeground,
    light: buttonForeground,
    hcDark: null,
    hcLight: null
}, ( localize(8019, "Button foreground color for extension actions.")));
registerColor('extensionButton.hoverBackground', {
    dark: buttonHoverBackground,
    light: buttonHoverBackground,
    hcDark: null,
    hcLight: null
}, ( localize(8020, "Button background hover color for extension actions.")));
registerColor('extensionButton.separator', buttonSeparator, ( localize(8021, "Button separator color for extension actions")));
