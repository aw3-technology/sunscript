...
import { REPL_VIEW_ID } from '../common/debug.js';
...
async function showDebugSessionMenu(accessor, selectAndStartID) {
    ...
    quickPick.placeholder = ( localize(10214, 'Search debug sessions by name'));
...
function _getPicksAndActiveItem(filter, selectAndStartID, debugService, viewsService, commandService) {
    ...
    const createDebugSessionLabel = ( localize(10215, 'Start a New Debug Session'));
...
function _getSessionInfo(session) {
    ...
    let ariaLabel = '';
    if (parentName) {
        ariaLabel = ( localize(10216, 'Session {0} spawned from {1}', label, parentName));
