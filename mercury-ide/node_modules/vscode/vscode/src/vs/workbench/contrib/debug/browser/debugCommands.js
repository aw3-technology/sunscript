...
import { CONTEXT_DISASSEMBLY_VIEW_FOCUS, CONTEXT_JUMP_TO_CURSOR_SUPPORTED, CONTEXT_IN_DEBUG_REPL, CONTEXT_IN_DEBUG_MODE, CONTEXT_DEBUG_STATE, CONTEXT_STEP_INTO_TARGETS_SUPPORTED, CONTEXT_FOCUSED_SESSION_IS_ATTACH, REPL_VIEW_ID, State, CONTEXT_DEBUGGERS_AVAILABLE, getStateLabel, CONTEXT_BREAKPOINTS_FOCUSED, CONTEXT_WATCH_EXPRESSIONS_FOCUSED, CONTEXT_VARIABLES_FOCUSED, CONTEXT_EXPRESSION_SELECTED, CONTEXT_BREAKPOINT_INPUT_FOCUSED, VIEWLET_ID as VIEWLET_ID$1, isFrameDeemphasized, EDITOR_CONTRIBUTION_ID } from '../common/debug.js';
...
const DEBUG_COMMAND_CATEGORY = ( localize2(5750, 'Debug'));
const RESTART_LABEL = ( localize2(5751, "Restart"));
const STEP_OVER_LABEL = ( localize2(5752, "Step Over"));
const STEP_INTO_LABEL = ( localize2(5753, "Step Into"));
const STEP_INTO_TARGET_LABEL = ( localize2(5754, "Step Into Target"));
const STEP_OUT_LABEL = ( localize2(5755, "Step Out"));
const PAUSE_LABEL = ( localize2(5756, "Pause"));
const DISCONNECT_LABEL = ( localize2(5757, "Disconnect"));
const DISCONNECT_AND_SUSPEND_LABEL = ( localize2(5758, "Disconnect and Suspend"));
const STOP_LABEL = ( localize2(5759, "Stop"));
const CONTINUE_LABEL = ( localize2(5760, "Continue"));
const FOCUS_SESSION_LABEL = ( localize2(5761, "Focus Session"));
const SELECT_AND_START_LABEL = ( localize2(5762, "Select and Start Debugging"));
const DEBUG_CONFIGURE_LABEL = ( localize(5763, "Open '{0}'", 'launch.json'));
const DEBUG_START_LABEL = ( localize2(5764, "Start Debugging"));
const DEBUG_RUN_LABEL = ( localize2(5765, "Start Without Debugging"));
const NEXT_DEBUG_CONSOLE_LABEL = ( localize2(5766, "Focus Next Debug Console"));
const PREV_DEBUG_CONSOLE_LABEL = ( localize2(5767, "Focus Previous Debug Console"));
const OPEN_LOADED_SCRIPTS_LABEL = ( localize2(5768, "Open Loaded Script..."));
const CALLSTACK_TOP_LABEL = ( localize2(5769, "Navigate to Top of Call Stack"));
const CALLSTACK_BOTTOM_LABEL = ( localize2(5770, "Navigate to Bottom of Call Stack"));
const CALLSTACK_UP_LABEL = ( localize2(5771, "Navigate Up Call Stack"));
const CALLSTACK_DOWN_LABEL = ( localize2(5772, "Navigate Down Call Stack"));
const COPY_EVALUATE_PATH_LABEL = ( localize2(5773, "Copy as Expression"));
const COPY_VALUE_LABEL = ( localize2(5774, "Copy Value"));
const ADD_TO_WATCH_LABEL = ( localize2(5775, "Add to Watch"));
const SELECT_DEBUG_CONSOLE_LABEL = ( localize2(5776, "Select Debug Console"));
const SELECT_DEBUG_SESSION_LABEL = ( localize2(5777, "Select Debug Session"));
...
CommandsRegistry.registerCommand({
    ...
    handler: async (accessor) => {
            ...
            if (source) {
                ...
                if (targets && targets.length) {
                    ...
                    if (targets.length > 1) {
                        ...
                        const pick = await quickInputService.pick(picks, { placeHolder: ( localize(5778, "Choose the specific location")) });
                        if (!pick) {
                            return;
                        }
                        id = pick._id;
        ...
        return notificationService.warn(( localize(5779, "No executable code is associated at the current cursor position.")));
    }
});
CommandsRegistry.registerCommand({
    id: CALLSTACK_TOP_ID,
...
MenuRegistry.appendMenuItem(MenuId.EditorContext, {
    command: {
        id: JUMP_TO_CURSOR_ID,
        title: ( localize(5780, "Jump to Cursor")),
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    mac: { primary: KeyMod$1.Shift | KeyMod$1.CtrlCmd | KeyCode.BracketRight },
    handler: async (accessor, _, context) => {
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    mac: { primary: KeyMod$1.Shift | KeyMod$1.CtrlCmd | KeyCode.BracketLeft },
    handler: async (accessor, _, context) => {
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    primary: KeyMod$1.Shift | KeyMod$1.CtrlCmd | KeyCode.F5,
...
const STEP_INTO_KEYBINDING = (isWeb && isWindows) ? (KeyMod$1.Alt | KeyCode.F11) : KeyCode.F11;
KeybindingsRegistry.registerCommandAndKeybindingRule({
    id: STEP_INTO_ID,
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    id: STEP_OUT_ID,
    weight: KeybindingWeight.WorkbenchContrib,
    primary: KeyMod$1.Shift | KeyCode.F11,
    when: ( (CONTEXT_DEBUG_STATE.isEqualTo('stopped'))),
    handler: async (accessor, _, context) => {
        const contextKeyService = accessor.get(IContextKeyService);
        if (CONTEXT_DISASSEMBLY_VIEW_FOCUS.getValue(contextKeyService)) {
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    primary: STEP_INTO_KEYBINDING | KeyMod$1.CtrlCmd,
    ...
    handler: async (accessor, _, context) => {
        ...
        session.stepInTargets(frame.frameId).then(targets => {
            ...
            if (targets?.length) {
                ...
                qp.placeholder = ( localize(5781, "No step targets available"));
            }
        });
    }
});
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    id: DISCONNECT_ID,
    weight: KeybindingWeight.WorkbenchContrib,
    primary: KeyMod$1.Shift | KeyCode.F5,
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    weight: KeybindingWeight.WorkbenchContrib,
    primary: KeyMod$1.CtrlCmd | KeyCode.F5,
    mac: { primary: KeyMod$1.WinCtrl | KeyCode.F5 },
    when: ( (ContextKeyExpr.and(
        CONTEXT_DEBUGGERS_AVAILABLE,
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    mac: { primary: KeyMod$1.CtrlCmd | KeyCode.Backspace },
    handler: (accessor, expression) => {
        const debugService = accessor.get(IDebugService);
        if (expression instanceof Expression) {
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    handler: async (accessor, query) => {
        ...
        const viewlet = (await paneCompositeService.openPaneComposite(VIEWLET_ID, ViewContainerLocation.Sidebar, true))?.getViewPaneContainer();
...
registerAction2(class AddConfigurationAction extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(5782, "Add Configuration...")),
            category: DEBUG_COMMAND_CATEGORY,
            f1: true,
            menu: {
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    weight: KeybindingWeight.WorkbenchContrib,
    primary: KeyMod$1.Shift | KeyCode.F9,
    when: EditorContextKeys.editorTextFocus,
...
MenuRegistry.appendMenuItem(MenuId.EditorContext, {
    command: {
        id: TOGGLE_INLINE_BREAKPOINT_ID,
        title: ( localize(5783, "Add Inline Breakpoint")),
        category: DEBUG_COMMAND_CATEGORY
    },
    when: ( (ContextKeyExpr.and(
        CONTEXT_IN_DEBUG_MODE,
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    secondary: [KeyMod$1.Alt | KeyCode.Enter],
    handler: (accessor) => {
        const listService = accessor.get(IListService);
        const list = listService.lastFocusedList;
        if (list instanceof List) {
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    primary: KeyCode.F5,
    secondary: [KeyMod$1.CtrlCmd | KeyCode.F5],
    handler: async (accessor) => {
        const paneCompositeService = accessor.get(IPaneCompositePartService);
        await paneCompositeService.openPaneComposite(VIEWLET_ID$1, ViewContainerLocation.Sidebar, true);
    }
});
