...
const COMMENTS_VIEW_TITLE = ( localize2(5319, "Comments"));
...
let CommentNodeRenderer = class CommentNodeRenderer {
    ...
    getCountString(commentCount) {
        if (commentCount > 2) {
            return ( localize(5320, "{0} replies", commentCount - 1));
        }
        else if (commentCount === 2) {
            return ( localize(5321, "1 reply"));
        }
        else {
            return ( localize(5322, "1 comment"));
        }
    }
    getRenderedComment(commentBody, disposables) {
        ...
        for (let i = 0; i < images.length; i++) {
            ...
            textDescription.textContent = image.alt ? ( localize(5323, "Image: {0}", image.alt)) : ( localize(5324, "Image"));
    ...
    renderElement(node, index, templateData, height) {
        ...
        if (node.element.threadRelevance === CommentThreadApplicability.Outdated) {
            ...
            templateData.threadMetadata.relevance.innerText = ( localize(5325, "Outdated"));
        ...
        if (node.element.range) {
            if (node.element.range.startLineNumber === node.element.range.endLineNumber) {
                templateData.threadMetadata.range.textContent = ( localize(5326, "[Ln {0}]", node.element.range.startLineNumber));
            }
            else {
        ...
        templateData.repliesMetadata.lastReplyDetail.textContent = ( localize(5328, "Last reply from {0}", lastComment.userName));
