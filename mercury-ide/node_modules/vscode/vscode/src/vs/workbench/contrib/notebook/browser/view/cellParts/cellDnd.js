import { append, addDisposableListener, EventType, isAncestor, $ as $$1 } from '../../../../../../base/browser/dom.js';
...
class CellDragAndDropController extends Disposable {
    constructor(notebookEditor, notebookListContainer) {
        ...
        this.listInsertionIndicator = append(notebookListContainer, $('.cell-list-insertion-indicator'));
        this._register(addDisposableListener(notebookListContainer.ownerDocument.body, EventType.DRAG_START, this.onGlobalDragStart.bind(this), true));
        this._register(addDisposableListener(notebookListContainer.ownerDocument.body, EventType.DRAG_END, this.onGlobalDragEnd.bind(this), true));
        const addCellDragListener = (eventType, handler, useCapture = false) => {
            this._register(addDisposableListener(notebookEditor.getDomNode(), eventType, e => {
        ...
        addCellDragListener(EventType.DRAG_OVER, event => {
        ...
        addCellDragListener(EventType.DROP, event => {
            if (!this.currentDraggedCell) {
                return;
            }
            event.browserEvent.preventDefault();
        ...
        addCellDragListener(EventType.DRAG_LEAVE, event => {
            event.browserEvent.preventDefault();
    ...
    _dropImpl(draggedCell, dropDirection, ctx, draggedOverCell) {
        ...
        const isCopy = (ctx.ctrlKey && !isMacintosh) || (ctx.altKey && isMacintosh);
            ...
            textModel.applyEdits([
                {
                    ...
                    cells: ( cellRangesToIndexes([range]).map(
                        index => cloneNotebookCellTextModel(this.notebookEditor.cellAt(index).model)
                    ))
                }
    ...
    onCellDragLeave(event) {
        if (!event.browserEvent.relatedTarget || !isAncestor(event.browserEvent.relatedTarget, this.notebookEditor.getDomNode())) {
            this.setInsertIndicatorVisibility(false);
        }
    }
    dragCleanup() {
    ...
    registerDragHandle(templateData, cellRoot, dragHandles, dragImageProvider) {
        ...
        for (const dragHandle of dragHandles) {
            templateData.templateDisposables.add(addDisposableListener(dragHandle, EventType.DRAG_END, onDragEnd));
        }
        ...
        for (const dragHandle of dragHandles) {
            templateData.templateDisposables.add(addDisposableListener(dragHandle, EventType.DRAG_START, onDragStart));
        }
    }
...
export { CellDragAndDropController, CellDragAndDropPart, performCellDropEdits };
