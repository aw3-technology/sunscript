...
import { ByteSize, FileSystemProviderCapabilities } from '../../../../platform/files/common/files.js';
...
import { VIEW_ID, UndoConfirmLevel } from '../common/files.js';
import { IEditorService } from '../../../services/editor/common/editorService.service.js';
import { Promises, RunOnceWorker, Limiter } from '../../../../base/common/async.js';
...
import { isDragEvent, getActiveWindow, triggerDownload } from '../../../../base/browser/dom.js';
...
let BrowserFileUpload = class BrowserFileUpload {
    ...
    upload(target, source) {
        ...
        const uploadPromise = this.progressService.withProgress({
            ...
            delay: 800,
            cancellable: true,
            title: ( localize(10019, "Uploading"))
            ...
            return uploadLimiter.queue(async () => {
                ...
                if (target && entry.name && target.getChild(entry.name)) {
                    ...
                    await this.explorerService.applyBulkEdit([( (new ResourceFileEdit(
                        ...
                        undoLabel: ( localize(10020, "Overwrite {0}", entry.name)),
                        progressLabel: ( localize(10021, "Overwriting {0}", entry.name)),
                    });
                    if (token.isCancellationRequested) {
                        return;
                    }
    ...
    async doImport(target, source, targetWindow, token) {
        ...
        if (folders.length > 0 && target.isRoot) {
            ...
            const buttons = [
                {
                    label: folders.length > 1 ?
                        ( localize(10025, "&&Copy Folders")) :
                        ( localize(10026, "&&Copy Folder")),
            ...
            if (( (folders.some(folder => workspaceFolderSchemas.indexOf(folder.uri.scheme) >= 0)))) {
                buttons.unshift({
                    label: folders.length > 1 ?
                        ( localize(10027, "&&Add Folders to Workspace")) :
                        ( localize(10028, "&&Add Folder to Workspace")),
                ...
                message = folders.length > 1 ?
                    ( localize(10031, "Are you sure to want to copy folders?")) :
                    ( localize(10032, "Are you sure to want to copy '{0}'?", basename(folders[0].uri)));
            }
    ...
    async importResources(target, resources, token) {
        if (resources && resources.length > 0) {
            ...
            if (inaccessibleFileCount > 0) {
                ...
                )) : ( localize(10034, "The dropped file could not be accessed for import.")));
            }
            const resourceFileEdits = ( (resourcesFiltered.map(resource => {
                ...
                return (
                     (new ResourceFileEdit(resource, targetFile, { overwrite: true, copy: true }))
                );
            })));
            const undoLevel = this.configurationService.getValue().explorer.confirmUndo;
            await this.explorerService.applyBulkEdit(resourceFileEdits, {
                undoLabel: resourcesFiltered.length === 1 ?
                    ( localize(10035, "Import {0}", basename(resourcesFiltered[0]))) :
                    ( localize(10036, "Import {0} resources", resourcesFiltered.length)),
                progressLabel: resourcesFiltered.length === 1 ?
                    ( localize(10037, "Copying {0}", basename(resourcesFiltered[0]))) :
                    ( localize(10038, "Copying {0} resources", resourcesFiltered.length)),
...
let FileDownload = class FileDownload {
    ...
    download(source) {
        ...
        const downloadPromise = this.progressService.withProgress({
            ...
            delay: 800,
            cancellable: isWeb,
            title: ( localize(10039, "Downloading"))
    ...
    async doDownloadNative(explorerItem, progress, cts) {
        ...
        let defaultUri;
        const lastUsedDownloadPath = this.storageService.get(FileDownload_1.LAST_USED_DOWNLOAD_PATH_STORAGE_KEY, StorageScope.APPLICATION);
        ...
        const destination = await this.fileDialogService.showSaveDialog({
            ...
            saveLabel: mnemonicButtonLabel(( localize(10042, "Download"))),
            title: ( localize(10043, "Choose Where to Download")),
            defaultUri
        });
        if (destination) {
            this.storageService.store(FileDownload_1.LAST_USED_DOWNLOAD_PATH_STORAGE_KEY, dirname(destination).fsPath, StorageScope.APPLICATION, StorageTarget.MACHINE);
            await this.explorerService.applyBulkEdit([( (new ResourceFileEdit(explorerItem.resource, destination, { overwrite: true, copy: true })))], {
                undoLabel: ( localize(10044, "Download {0}", explorerItem.name)),
                progressLabel: ( localize(10045, "Downloading {0}", explorerItem.name)),
                progressLocation: ProgressLocation.Window
            });
        }
        else {
...
function getFileOverwriteConfirm(name) {
    return {
        ...
        detail: ( localize(10047, "This action is irreversible!")),
        primaryButton: ( localize(10048, "&&Replace")),
        type: 'warning'
    };
}
function getMultipleFilesOverwriteConfirm(files) {
    if (files.length > 1) {
        return {
            ...
            detail: getFileNamesMessage(files) + '\n' + ( localize(10047, "This action is irreversible!")),
            primaryButton: ( localize(10048, "&&Replace")),
            type: 'warning'
        };
    }
