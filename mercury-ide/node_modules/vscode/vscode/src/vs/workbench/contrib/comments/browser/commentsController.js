...
import { CommentWidgetFocus, ReviewZoneWidget, parseMouseDownInfoFromEvent, isMouseUpEventDragFromMouseDown } from './commentThreadZoneWidget.js';
...
let CommentController = class CommentController {
    ...
    _findNearestCommentingRange(reverse) {
        ...
        if (this.accessibilityService.isScreenReaderOptimized()) {
            ...
            if (commentRangeStart && commentRangeEnd) {
                ...
                oneLine ? status(( localize(5346, "Line {0}", commentRangeStart))) : status(( localize(5347, "Lines {0} to {1}", commentRangeStart, commentRangeEnd)));
            }
        }
    }
    nextCommentingRange() {
            ...
            if (!this._hasRespondedToEditorChange) {
                if (( (this._commentInfos.some(
                    ...
                    if (verbose) {
                        ...
                        status(( localize(5350, "Editor has commenting ranges.")));
                    }
                }
            }
        });
    ...
    async openCommentsView(thread) {
        if (thread.comments && (thread.comments.length > 0)) {
            ...
            else if (openViewState === 'firstFile' || (openViewState === 'firstFileUnresolved' && thread.state === CommentThreadState.Unresolved)) {
    ...
    addCommentAtLine(range, e) {
        ...
        if (newCommentInfos.length > 1) {
            if (e && range) {
                ...
                return this.quickInputService.pick(picks, { placeHolder: ( localize(5351, "Select Comment Provider")), matchOnDescription: true }).then(pick => {
                    if (!pick) {
                        return;
                    }
...
export { CommentController, ID, revealCommentThread };
