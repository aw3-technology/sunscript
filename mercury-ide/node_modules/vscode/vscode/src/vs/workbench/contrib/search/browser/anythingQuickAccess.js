...
import { QuickPickItemScorerAccessor, QuickInputHideReason, quickPickItemScorerAccessor } from '../../../../platform/quickinput/common/quickInput.js';
...
import { toLocalResource, basenameOrAuthority, dirname } from '../../../../base/common/resources.js';
...
import { IQuickChatService } from '../../chat/browser/chat.service.js';
...
let AnythingQuickAccessProvider = class AnythingQuickAccessProvider extends PickerQuickAccessProvider {
    ...
    static { this.NO_RESULTS_PICK = {
        label: ( localize(8281, "No matching results"))
    }; }
    ...
    doGetPicks(filter, options, disposables, token) {
        ...
        if (this.pickState.isQuickNavigating) {
            if (picks.length > 0) {
                picks.push({ type: 'separator', label: ( localize(8282, "recently opened")) });
            }
        ...
        return {
            ...
            additionalPicks: (async () => {
                ...
                return additionalPicks.length > 0 ? [
                    { type: 'separator', label: this.configuration.includeSymbols ? ( localize(8283, "file and symbol results")) : ( localize(8284, "file results")) },
                    ...additionalPicks
                ] : [];
            })(),
    ...
    getHelpPicks(query, token, runOptions) {
        ...
        const providers = this.lazyRegistry.value.getQuickAccessProviders()
            ...
            return {
                ...
                ariaLabel: ( localize(8285, "{0}, {1}", label, helpEntry.description)),
                accept: () => {
        ...
        if (this.quickChatService.enabled) {
            providers.push({
                label: ( localize(8286, "Open Quick Chat")),
    ...
    createAnythingPick(resourceOrEditor, configuration) {
        ...
        return {
            resource,
            label,
            ariaLabel: isDirty ? ( localize(8290, "{0} unsaved changes", labelAndDescription)) : labelAndDescription,
            description,
