...
import { decodeBase64, VSBuffer, encodeBase64 } from '../../../../base/common/buffer.js';
...
import { DebugTreeItemCollapsibleState, DEBUG_MEMORY_SCHEME, DataBreakpointSetType, isFrameDeemphasized, State, MemoryRangeType } from './debug.js';
...
class ExpressionContainer {
    ...
    async fetchVariables(start, count, filter) {
        try {
            ...
            const vars = ( (response.body.variables.filter(v => !!v).map((v) => {
                ...
                return (
                     (new Variable(this.session, this.threadId, this, 0, '', undefined, ( localize(5810, "Invalid variable attributes")), 0, 0, undefined, { kind: 'virtual' }, undefined, undefined, false))
                );
            })));
    ...
    async evaluateExpression(expression, session, stackFrame, context, keepLazyVars = false, location) {
        if (!session || (!stackFrame && context !== 'repl')) {
            this.value = context === 'repl' ? ( localize(5811, "Please start a debug session to evaluate expressions")) : Expression.DEFAULT_VALUE;
            this.reference = 0;
            return false;
        }
        this.session = session;
...
class Expression extends ExpressionContainer {
    static { this.DEFAULT_VALUE = ( localize(5812, "not available")); }
    ...
    get stateLabel() {
        if (this.stoppedDetails) {
            return this.stoppedDetails.description ||
                (this.stoppedDetails.reason ? ( localize(5813, "Paused on {0}", this.stoppedDetails.reason)) : ( localize(5814, "Paused")));
        }
        return ( localize(5815, "Running"));
    }
...
let DebugModel = class DebugModel extends Disposable {
    constructor(debugStorage, textFileService, uriIdentityService, logService) {
        ...
        this.watchExpressionChangeListeners = this._register(( (new DisposableMap())));
...
export { BaseBreakpoint, Breakpoint, DataBreakpoint, DebugModel, Enablement, ErrorScope, ExceptionBreakpoint, Expression, ExpressionContainer, FunctionBreakpoint, InstructionBreakpoint, MemoryRegion, Scope, StackFrame, Thread, ThreadAndSessionIds, Variable, VisualizedExpression, getUriForDebugMemory };
