...
function toKernelQuickPick(kernel, selected) {
    ...
    if (kernel.id === selected?.id) {
        if (!res.description) {
            res.description = ( localize(8061, "Currently Selected"));
        }
        else {
            res.description = ( localize(8062, "{0} - Currently Selected", res.description));
        }
    }
    return res;
}
class KernelPickerStrategyBase {
    ...
    async showQuickPick(editor, wantedId, skipAutoRun) {
        ...
        quickPick.placeholder = selected
            ...
            8063,
            "Change kernel for '{0}'",
            this._labelService.getUriLabel(notebook.uri, { relative: true })
        ))
            : ( localize(
            8064,
            "Select kernel for '{0}'",
    ...
    async _getKernelRecommendationsQuickPickItems(notebookTextModel, extensionWorkbenchService) {
        ...
        if (suggestedExtension) {
            ...
            const extensions = extensionWorkbenchService.installed.filter(e => (e.enablementState === EnablementState.EnabledByEnvironment || e.enablementState === EnablementState.EnabledGlobally || e.enablementState === EnablementState.EnabledWorkspace)
            ...
            quickPickItems.push({
                ...
                label: `$(${Codicon.lightbulb.id}) ` + ( localize(8065, 'Install/Enable suggested extensions')),
                extensionIds: suggestedExtension.extensionIds
            });
        }
        quickPickItems.push({
            id: 'install',
            label: ( localize(8066, "Browse marketplace for kernel extensions")),
        });
...
let KernelPickerMRUStrategy = class KernelPickerMRUStrategy extends KernelPickerStrategyBase {
    ...
    _getKernelPickerQuickPickItems(notebookTextModel, matchResult, notebookKernelService, scopedContextKeyService) {
        ...
        quickPickItems.push({
            id: 'selectAnother',
            label: ( localize(8067, "Select Another Kernel...")),
    ...
    async displaySelectAnotherQuickPick(editor, kernelListEmpty) {
        ...
        const quickPickItem = await ( (new Promise(resolve => {
            quickPick.title = kernelListEmpty ? ( localize(8068, "Select Kernel")) : ( localize(8069, "Select Another Kernel"));
            quickPick.placeholder = ( localize(8070, "Type to choose a kernel source"));
            quickPick.busy = true;
    ...
    async _calculdateKernelSources(editor) {
        ...
        quickPickItems.push(...( (validActions.map(action => {
            const buttons = action.documentation ? [{
                    ...
                    tooltip: ( localize(8071, 'Learn More')),
                }] : [];
            return {
    ...
    async _selectOneKernel(notebook, source, kernels) {
        ...
        quickPick.items = quickPickItems;
        quickPick.canSelectMany = false;
        quickPick.title = ( localize(8072, "Select Kernel from {0}", source));
    ...
    static updateKernelStatusAction(notebook, action, notebookKernelService, notebookKernelHistoryService) {
        ...
        if (detectionTasks.length) {
            ...
            if (info.selected) {
                ...
                action.tooltip = kernelInfo
                    ? ( localize(8073, "Selected Kernel: {0} (Kernel Detection Tasks Running)", kernelInfo))
                    : ( localize(8074, "Detecting Kernels"));
            }
            else {
                action.label = ( localize(8074, "Detecting Kernels"));
            }
            return;
        }
        ...
        if (runningActions.length) {
            return updateActionFromSourceAction(runningActions[0] );
        }
        ...
        if (selected) {
            ...
            action.label = ( localize(8068, "Select Kernel"));
            action.class = ThemeIcon.asClassName(selectKernelIcon);
            action.tooltip = '';
        }
    }
