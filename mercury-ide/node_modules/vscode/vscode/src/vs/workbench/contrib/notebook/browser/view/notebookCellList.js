...
import { scheduleAtNextAnimationFrame, getWindow, isAncestorOfActiveElement, findParentWithClass, createStyleSheet } from '../../../../../base/browser/dom.js';
...
import { CellEditState, CellFocusMode, CellRevealType, CellRevealRangeType, CursorAtBoundary, CursorAtLineBoundary } from '../notebookBrowser.js';
import { NOTEBOOK_EDITOR_CURSOR_BOUNDARY, NOTEBOOK_EDITOR_CURSOR_LINE_BOUNDARY, CellKind, diff, SelectionStateType } from '../../common/notebookCommon.js';
import { cellRangesEqual, reduceCellRanges, cellRangesToIndexes } from '../../common/notebookRange.js';
...
const NOTEBOOK_WEBVIEW_BOUNDARY = 5000;
...
let NotebookCellList = class NotebookCellList extends WorkbenchList {
    ...
    constructor(listUser, container, notebookOptions, delegate, renderers, contextKeyService, options, listService, configurationService, instantiationService, notebookExecutionStateService) {
        ...
        this._localDisposableStore.add(this.view.onDidChangeContentHeight(() => {
            if (this._isInLayout) {
                scheduleAtNextAnimationFrame(getWindow(container), () => {
                    updateVisibleRanges();
                });
            }
            updateVisibleRanges();
    ...
    attachViewModel(model) {
        ...
        this._viewModelStore.add(model.onDidChangeViewCells((e) => {
            ...
            if (e.synchronous) {
                ...
                this._viewModelStore.add(scheduleAtNextAnimationFrame(getWindow(this.rowsContainer), () => {
                    if (this._isDisposed) {
                        return;
                    }
                    this._updateElementsInWebview(viewDiffs);
    ...
    setHiddenAreas(_ranges, triggerViewUpdate) {
        ...
        const hiddenAreaIds = ( newRanges.map(
            range => this._viewModel.setTrackedRange(null, range, TrackedRangeStickiness.GrowsOnlyWhenTypingAfter)
        )).filter(id => id !== null);
    ...
    splice2(start, deleteCount, elements = []) {
        ...
        const focusInside = isAncestorOfActiveElement(this.rowsContainer);
    ...
    domFocus() {
        ...
        if (!isMacintosh && this.view.domNode.ownerDocument.activeElement && !!findParentWithClass(this.view.domNode.ownerDocument.activeElement, 'context-view')) {
            return;
        }
        super.domFocus();
    }
...
export { ListViewInfoAccessor, NOTEBOOK_WEBVIEW_BOUNDARY, NotebookCellList };
