...
import { clearNode, append, $, addStandardDisposableListener, EventType, addDisposableListener } from '../../../../base/browser/dom.js';
...
import { DisposableStore, toDisposable, Disposable, dispose } from '../../../../base/common/lifecycle.js';
...
import { TUNNEL_VIEW_ID, TunnelType, TunnelEditId } from '../../../services/remote/common/remoteExplorerService.js';
...
import { MessageType, InputBox } from '../../../../base/browser/ui/inputbox/inputBox.js';
...
import { TunnelProtocol, TunnelPrivacyId, isLocalhost, isAllInterfaces } from '../../../../platform/tunnel/common/tunnel.js';
...
import { labelPortIcon, forwardPortIcon, stopForwardIcon, copyAddressIcon, openBrowserIcon, openPreviewIcon, privatePortIcon, forwardedPortWithProcessIcon, forwardedPortWithoutProcessIcon, portsViewIcon } from './remoteIcons.js';
...
import { makeAddress, forwardedPortsViewEnabled, parseAddress, TunnelCloseReason, mapHasAddressLocalhostOrAllInterfaces, TunnelSource } from '../../../services/remote/common/tunnelModel.js';
...
class PortColumn {
    constructor() {
        this.label = ( localize(10857, "Port"));
        this.tooltip = ( localize(10858, "The label and remote port number of the forwarded port."));
        this.weight = 1;
        this.templateId = 'actionbar';
    }
    project(row) {
...
class LocalAddressColumn {
    constructor() {
        this.label = ( localize(10859, "Forwarded Address"));
        this.tooltip = ( localize(10860, "The address that the forwarded port is available at."));
        this.weight = 1;
        this.templateId = 'actionbar';
    }
    project(row) {
    ...
    static getHoverText(localAddress) {
        return function (configurationService) {
            ...
            if (editorConf.multiCursorModifier === 'ctrlCmd') {
                if (isMacintosh) {
                    clickLabel = ( localize(10861, "option + click"));
                }
                else {
                    clickLabel = ( localize(10862, "alt + click"));
                }
            }
            else {
                if (isMacintosh) {
                    clickLabel = ( localize(10863, "cmd + click"));
                }
                else {
                    clickLabel = ( localize(10864, "ctrl + click"));
                }
            }
...
class RunningProcessColumn {
    constructor() {
        this.label = ( localize(10865, "Running Process"));
        this.tooltip = ( localize(10866, "The command line of the process that is using the port."));
        this.weight = 2;
        this.templateId = 'actionbar';
    }
    project(row) {
...
class OriginColumn {
    constructor() {
        this.label = ( localize(10867, "Origin"));
...
class PrivacyColumn {
    constructor() {
        this.label = ( localize(10869, "Visibility"));
        this.tooltip = ( localize(10870, "The availability of the forwarded port."));
        this.weight = 1;
        this.templateId = 'actionbar';
    }
    project(row) {
...
let ActionBarRenderer = class ActionBarRenderer extends Disposable {
    ...
    renderInputBox(container, editableData) {
        ...
        const inputBox = ( (new InputBox(container, this.contextViewService, {
            ariaLabel: ( localize(10871, "Press Enter to confirm or Escape to cancel.")),
            validationOptions: {
                validation: (value) => {
                    ...
                    return {
                        ...
                        formatContent: true,
                        type: message.severity === Severity$1.Error ? MessageType.ERROR : MessageType.INFO
                    };
                }
            },
...
class TunnelItem {
    ...
    get processDescription() {
        ...
        if (this.runningProcess) {
            ...
            description = ( localize(10872, "Process information unavailable"));
        }
        return description;
    }
    get tooltipPostfix() {
    ...
    get iconTooltip() {
        ...
        if (!isAdd) {
            return `${this.processDescription ? ( localize(10875, "Port has running process.")) :
                ( localize(10876, "No running process."))}`;
        }
        else {
            return this.label;
        }
    ...
    get portTooltip() {
        const isAdd = this.tunnelType === TunnelType.Add;
        if (!isAdd) {
            return `${this.name ? ( localize(10877, "Port labeled {0}. ", this.name)) : ''}`;
        }
        else {
            return '';
        }
...
const TunnelViewFocusContextKey = ( (new RawContextKey('tunnelViewFocus', false, ( localize(10879, "Whether the Ports view has focus.")))));
...
let TunnelPanel = class TunnelPanel extends ViewPane {
    ...
    static { this.TITLE = ( localize2(10880, "Ports")); }
    ...
    createTable() {
        ...
        this.table = this.instantiationService.createInstance(WorkbenchTable, 'RemoteTunnels', widgetContainer, ( (new TunnelTreeVirtualDelegate(this.remoteExplorerService))), columns, [actionBarRenderer], {
            ...
            accessibilityProvider: {
                ...
                getWidgetAriaLabel: () => ( localize(10881, "Tunnel View"))
            },
...
( ((function(LabelTunnelAction) {
    ...
    LabelTunnelAction.LABEL = ( localize(10882, "Set Port Label"));
                ...
                return (
                     (new Promise(resolve => {
                        ...
                        remoteExplorerService.setEditable(tunnelItem, TunnelEditId.Label, {
                            ...
                            placeholder: ( localize(10883, "Port label")),
                            startingValue
                        });
                    }))
                );
...
const invalidPortString = ( localize(10884, "Forwarded port should be a number or a host:port."));
const maxPortNumber = 65536;
const invalidPortNumberString = ( localize(10885, "Port number must be \u2265 0 and < {0}.", maxPortNumber));
const requiresSudoString = ( localize(10886, "May Require Sudo"));
const alreadyForwarded = ( localize(10887, "Port is already forwarded"));
...
( ((function(ForwardPortAction) {
    ...
    ForwardPortAction.LABEL = ( localize2(10888, "Forward a Port"));
    ForwardPortAction.TREEITEM_LABEL = ( localize(10889, "Forward Port"));
    const forwardPrompt = ( localize(10890, "Port number or address (eg. 3000 or 10.10.10.10:2000)."));
    function validateInput(remoteExplorerService, tunnelService, value, canElevate) {
        const parsed = parseAddress(value);
        if (!parsed) {
            return { content: invalidPortString, severity: Severity$1.Error };
        }
        else if (parsed.port >= maxPortNumber) {
            return { content: invalidPortNumberString, severity: Severity$1.Error };
        }
        else if (canElevate && tunnelService.isPortPrivileged(parsed.port)) {
            return { content: requiresSudoString, severity: Severity$1.Info };
        }
        else if (mapHasAddressLocalhostOrAllInterfaces(remoteExplorerService.tunnelModel.forwarded, parsed.host, parsed.port)) {
            return { content: alreadyForwarded, severity: Severity$1.Error };
        }
        return null;
    }
    function error(notificationService, tunnelOrError, host, port) {
        if (!tunnelOrError) {
            ...
            notificationService.warn(( localize(10892, "Unable to forward {0}:{1}. {2}", host, port, tunnelOrError)));
        }
    }
...
( ((function(ClosePortAction) {
    ...
    ClosePortAction.LABEL = ( localize2(10894, "Stop Forwarding Port"));
    function inlineHandler() {
        return async (accessor, arg) => {
    ...
    function commandPaletteHandler() {
        return async (accessor) => {
            ...
            const result = await quickInputService.pick(picks, { placeHolder: ( localize(10895, "Choose a port to stop forwarding")) });
...
( ((function(OpenPortInBrowserAction) {
    ...
    OpenPortInBrowserAction.LABEL = ( localize(10896, "Open in Browser"));
    function handler() {
        return async (accessor, arg) => {
            let key;
            if (isITunnelItem(arg)) {
...
( ((function(OpenPortInPreviewAction) {
    ...
    OpenPortInPreviewAction.LABEL = ( localize(10897, "Preview in Editor"));
    function handler() {
        return async (accessor, arg) => {
            let key;
            if (isITunnelItem(arg)) {
...
( ((function(OpenPortInBrowserCommandPaletteAction) {
    ...
    OpenPortInBrowserCommandPaletteAction.LABEL = ( localize(10898, "Open Port in Browser"));
    function handler() {
        return async (accessor, arg) => {
            const remoteExplorerService = accessor.get(IRemoteExplorerService);
            const tunnelService = accessor.get(ITunnelService);
            ...
            if (options.length === 0) {
                options.push({
                    label: ( localize(10899, "No ports currently forwarded. Open the Ports view to get started."))
                });
            }
            else {
                options.push({
                    label: ( localize(10900, "Open the Ports view..."))
                });
            }
            const picked = await quickPickService.pick(options, { placeHolder: ( localize(10901, "Choose the port to open")) });
...
( ((function(CopyAddressAction) {
    ...
    CopyAddressAction.INLINE_LABEL = ( localize(10902, "Copy Local Address"));
    CopyAddressAction.COMMANDPALETTE_LABEL = ( localize(10903, "Copy Forwarded Port Address"));
    ...
    function commandPaletteHandler() {
        return async (accessor, arg) => {
            ...
            const result = await quickInputService.pick(makeTunnelPicks(tunnels, remoteExplorerService, tunnelService), { placeHolder: ( localize(10904, "Choose a forwarded port")) });
            if (result && result.tunnel) {
...
( ((function(ChangeLocalPortAction) {
    ...
    ChangeLocalPortAction.LABEL = ( localize(10905, "Change Local Address Port"));
    function validateInput(tunnelService, value, canElevate) {
        if (!value.match(/^[0-9]+$/)) {
            return { content: ( localize(10906, "Local port should be a number.")), severity: Severity$1.Error };
        }
        ...
        return async (accessor, arg) => {
            ...
            if (tunnelContext) {
                ...
                remoteExplorerService.setEditable(tunnelItem, TunnelEditId.LocalPort, {
                    ...
                    placeholder: ( localize(10908, "New local port"))
                });
            }
        };
    }
...
( ((function(SetTunnelProtocolAction) {
    ...
    SetTunnelProtocolAction.LABEL_HTTP = ( localize(10909, "HTTP"));
    SetTunnelProtocolAction.LABEL_HTTPS = ( localize(10910, "HTTPS"));
...
KeybindingsRegistry.registerCommandAndKeybindingRule({
    ...
    mac: {
        primary: KeyMod$1.CtrlCmd | KeyCode.Backspace,
...
MenuRegistry.appendMenuItem(MenuId.TunnelContext, ({
    ...
    title: ( localize(10911, "Port Visibility")),
...
MenuRegistry.appendMenuItem(MenuId.TunnelContext, ({
    ...
    title: ( localize(10912, "Change Port Protocol")),
