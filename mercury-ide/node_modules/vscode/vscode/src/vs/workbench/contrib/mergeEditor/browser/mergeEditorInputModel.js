        ...
        if (someDirty) {
            ...
            const buttons = [
                {
                    label: hasUnhandledConflicts ?
                        ( localize(10665, "&&Save With Conflicts")) :
                        ( localize(10666, "&&Save")),
                    run: () => ConfirmResult.SAVE
                },
                {
                    label: ( localize(10667, "Do&&n't Save")),
            ...
            choice = (await this.dialogService.prompt({
                ...
                detail: hasUnhandledConflicts
                    ...
                    : isMany
                        ? ( localize(10670, "The merge results will be lost if you don't save them."))
                        : ( localize(10671, "The merge result will be lost if you don't save it.")),
                buttons,
                cancelButton: {
    ...
    async save(options) {
        ...
        (async () => {
            const { confirmed } = await this.dialogService.confirm({
                message: ( localize(10672, "Do you want to accept the merge result?")),
                detail: ( localize(
                    10673,
                    "This will write the merge result to the original file and close the merge editor."
                )),
                primaryButton: ( localize(10674, '&&Accept Merge'))
            });
            if (confirmed) {
                await this.accept();
...
let WorkspaceMergeEditorModeFactory = class WorkspaceMergeEditorModeFactory {
    ...
    static { this.FILE_SAVED_SOURCE = SaveSourceRegistry.registerSource('merge-editor.source', ( localize(10675, "Before Resolving Conflicts In Merge Editor"))); }
    async createInputModel(args) {
        const store = ( (new DisposableStore()));
        ...
        if (someDirty) {
            ...
            const { result } = await this._dialogService.prompt({
                ...
                detail: someUnhandledConflicts ?
                    ...
                    : isMany
                        ? ( localize(10680, "Your changes will be lost if you don't save them."))
                        : ( localize(10681, "Your changes will be lost if you don't save them.")),
                buttons: [
                    {
                        label: someUnhandledConflicts
                            ? ( localize(10682, '&&Save with Conflicts'))
                            : ( localize(10683, '&&Save')),
                        run: () => ConfirmResult.SAVE
                    },
                    {
                        label: ( localize(10684, "Do&&n't Save")),
                        run: () => ConfirmResult.DONT_SAVE
                    }
                ],
                cancelButton: {
            ...
            const { confirmed, checkboxChecked } = await this._dialogService.confirm({
                message: isMany
                    ? ( localize(10685, 'Do you want to close {0} merge editors?', inputModels.length))
                    : ( localize(
                    10686,
                ...
                detail: someUnhandledConflicts ?
                    isMany
                        ? ( localize(10687, "The files contain unhandled conflicts."))
                        : ( localize(10688, "The file contains unhandled conflicts."))
                    : undefined,
                primaryButton: someUnhandledConflicts
                    ? ( localize(10689, '&&Close with Conflicts'))
                    : ( localize(10690, '&&Close')),
                checkbox: { label: ( localize(10691, "Do not ask me again")) }
            });
            if (checkboxChecked) {
