...
import { FileOperation, FileOperationResult, TooLargeFileOperationError, ByteSize, FileOperationError } from '../../../../../platform/files/common/files.js';
...
let TextFileEditor = class TextFileEditor extends AbstractTextCodeEditor {
    ...
    getTitle() {
        ...
        return ( localize(9780, "Text File Editor"));
    }
    get input() {
        return this._input;
    }
    ...
    async handleSetInputError(error, input, options) {
        ...
        if (error.fileOperationResult === FileOperationResult.FILE_IS_DIRECTORY) {
            ...
            actions.push(toAction({
                id: 'workbench.files.action.openFolder', label: ( localize(9781, "Open Folder")), run: async () => {
            ...
            if (this.contextService.isInsideWorkspace(input.preferredResource)) {
                actions.push(toAction({
                    id: 'workbench.files.action.reveal', label: ( localize(9782, "Reveal Folder")), run: async () => {
        ...
        if (error.fileOperationResult === FileOperationResult.FILE_NOT_FOUND &&
            ...
            const fileNotFoundError = createEditorOpenError(( (new FileOperationError(( localize(9786, "The editor could not be opened because the file was not found.")), FileOperationResult.FILE_NOT_FOUND))), [
                toAction({
                    id: 'workbench.files.action.createMissingFile', label: ( localize(9787, "Create File")), run: async () => {
