...
let CompositeBarActionViewItem = class CompositeBarActionViewItem extends BaseActionViewItem {
    ...
    computeTitle() {
        ...
        let title = this.keybindingLabel ? ( localize(10544, "{0} ({1})", this.compositeBarActionItem.name, this.keybindingLabel)) : this.compositeBarActionItem.name;
        const badge = this.action.activity?.badge;
        if (badge?.getDescription()) {
            title = ( localize(10545, "{0} - {1}", title, badge.getDescription()));
        }
        return title;
    }
...
class CompositeOverflowActivityAction extends CompositeBarAction {
    constructor(showMenu) {
        super({
            ...
            name: ( localize(10546, "Additional Views")),
    ...
    getActions() {
        return (
             (this.getOverflowingComposites().map(composite => {
                ...
                if (suffix) {
                    action.label = ( localize(10547, "{0} ({1})", composite.name, suffix));
                }
                else {
...
let ManageExtensionAction = class ManageExtensionAction extends Action {
    constructor(commandService) {
        super('activitybar.manage.extension', ( localize(10548, "Manage Extension")));
...
let CompositeActionViewItem = class CompositeActionViewItem extends CompositeBarActionViewItem {
    ...
    showContextMenu(container) {
        ...
        if (isPinned) {
            this.toggleCompositePinnedAction.label = ( localize(10549, "Hide '{0}'", this.compositeBarActionItem.name));
            this.toggleCompositePinnedAction.checked = false;
        }
        else {
            this.toggleCompositePinnedAction.label = ( localize(10550, "Keep '{0}'", this.compositeBarActionItem.name));
        }
        const isBadgeEnabled = this.compositeBar.areBadgesEnabled(this.compositeBarActionItem.id);
        if (isBadgeEnabled) {
            this.toggleCompositeBadgeAction.label = ( localize(10551, "Hide Badge"));
        }
        else {
            this.toggleCompositeBadgeAction.label = ( localize(10552, "Show Badge"));
        }
...
class ToggleCompositePinnedAction extends Action {
    constructor(activity, compositeBar) {
        super('show.toggleCompositePinned', activity ? activity.name : ( localize(10553, "Toggle View Pinned")));
...
class ToggleCompositeBadgeAction extends Action {
    constructor(compositeBarActionItem, compositeBar) {
        super('show.toggleCompositeBadge', compositeBarActionItem ? compositeBarActionItem.name : ( localize(10554, "Toggle View Badge")));
