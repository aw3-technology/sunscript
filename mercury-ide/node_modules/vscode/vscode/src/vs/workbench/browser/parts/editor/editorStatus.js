...
import { runAtThisOrScheduleAtNextAnimationFrame, getWindowById } from '../../../../base/browser/dom.js';
import { format, splitLines, compare } from '../../../../base/common/strings.js';
...
import { AutomaticLanguageDetectionLikelyWrongId } from '../../../services/languageDetection/common/languageDetectionWorkerService.js';
...
const nlsSingleSelectionRange = ( localize(9541, "Ln {0}, Col {1} ({2} selected)"));
const nlsSingleSelection = ( localize(9542, "Ln {0}, Col {1}"));
const nlsMultiSelectionRange = ( localize(9543, "{0} selections ({1} characters selected)"));
const nlsMultiSelection = ( localize(9544, "{0} selections"));
const nlsEOLLF = ( localize(9545, "LF"));
const nlsEOLCRLF = ( localize(9546, "CRLF"));
let EditorStatus = class EditorStatus extends Disposable {
    ...
    async showIndentationPicker() {
        ...
        if (!activeTextEditorControl) {
            return this.quickInputService.pick([{ label: ( localize(9547, "No text editor active at this time")) }]);
        }
        if (this.editorService.activeEditor?.isReadonly()) {
            return this.quickInputService.pick([{ label: ( localize(9548, "The active code editor is read-only.")) }]);
        }
        const picks = ( ([
        ...
        picks.splice(3, 0, { type: 'separator', label: ( localize(9549, "convert file")) });
        picks.unshift({ type: 'separator', label: ( localize(9550, "change view")) });
        const action = await this.quickInputService.pick(picks, { placeHolder: ( localize(9551, "Select Action")), matchOnDetail: true });
        return action?.run();
    }
    updateTabFocusModeElement(visible) {
        if (visible) {
            if (!this.tabFocusModeElement.value) {
                const text = ( localize(9552, "Tab Moves Focus"));
                this.tabFocusModeElement.value = this.statusbarService.addEntry({
                    name: ( localize(9553, "Accessibility Mode")),
                    text,
                    ariaLabel: text,
                    tooltip: ( localize(9554, "Disable Accessibility Mode")),
    ...
    updateColumnSelectionModeElement(visible) {
        if (visible) {
            if (!this.columnSelectionModeElement.value) {
                const text = ( localize(9555, "Column Selection"));
                this.columnSelectionModeElement.value = this.statusbarService.addEntry({
                    name: ( localize(9556, "Column Selection Mode")),
                    text,
                    ariaLabel: text,
                    tooltip: ( localize(9557, "Disable Column Selection Mode")),
    ...
    updateSelectionElement(text) {
        ...
        const props = {
            name: ( localize(9558, "Editor Selection")),
            text,
            ariaLabel: text,
            tooltip: ( localize(9559, "Go to Line/Column")),
    ...
    updateIndentationElement(text) {
        ...
        const props = {
            name: ( localize(9560, "Editor Indentation")),
            text,
            ariaLabel: text,
            tooltip: ( localize(9561, "Select Indentation")),
    ...
    updateEncodingElement(text) {
        ...
        const props = {
            name: ( localize(9562, "Editor Encoding")),
            text,
            ariaLabel: text,
            tooltip: ( localize(9563, "Select Encoding")),
    ...
    updateEOLElement(text) {
        ...
        const props = {
            name: ( localize(9564, "Editor End of Line")),
            text,
            ariaLabel: text,
            tooltip: ( localize(9565, "Select End of Line Sequence")),
    ...
    updateLanguageIdElement(text) {
        ...
        const props = {
            name: ( localize(9566, "Editor Language")),
            text,
            ariaLabel: text,
            tooltip: ( localize(9567, "Select Language Mode")),
    ...
    updateMetadataElement(text) {
        ...
        const props = {
            name: ( localize(9568, "File Information")),
            text,
            ariaLabel: text,
            tooltip: ( localize(9569, "File Information"))
        };
            ...
            if (model) {
                ...
                update.indentation = (modelOpts.insertSpaces
                    ? modelOpts.tabSize === modelOpts.indentSize
                        ? ( localize(9570, "Spaces: {0}", modelOpts.indentSize))
                        : ( localize(
                    9571,
                    "Spaces: {0} (Tab Size: {1})",
                    ...
                    : ( localize(9572, "Tab Size: {0}", modelOpts.tabSize)));
            }
        }
        ...
        if (this.hasToUpdateStatus(previousMarker, this.currentMarker)) {
            if (this.currentMarker) {
                ...
                if (!this.statusBarEntryAccessor.value) {
                    this.statusBarEntryAccessor.value = this.statusbarService.addEntry({ name: ( localize(9573, "Current Problem")), text: '', ariaLabel: '' }, 'statusbar.currentProblem', StatusbarAlignment.LEFT);
                }
                this.statusBarEntryAccessor.value.update({ name: ( localize(9573, "Current Problem")), text, ariaLabel: text });
            }
            else {
...
let ShowLanguageExtensionsAction = class ShowLanguageExtensionsAction extends Action {
    ...
    constructor(fileExtension, commandService, galleryService) {
        super(ShowLanguageExtensionsAction_1.ID, ( localize(9574, "Search Marketplace Extensions for '{0}'...", fileExtension)));
...
class ChangeLanguageAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(9575, 'Change Language Mode')),
            f1: true,
            keybinding: {
                weight: KeybindingWeight.WorkbenchContrib,
                primary: KeyChord(KeyMod$1.CtrlCmd | KeyCode.KeyK, KeyCode.KeyM)
            },
            precondition: ContextKeyExpr.not('notebookEditorFocused'),
            metadata: {
                description: ( localize(9576, "Change the language mode of the active text editor.")),
                args: [
                    {
                        name: ( localize(9577, "The name of the language mode to change to.")),
    ...
    async run(accessor, languageMode) {
        ...
        if (!activeTextEditorControl) {
            await quickInputService.pick([{ label: ( localize(9547, "No text editor active at this time")) }]);
            return;
        }
        ...
        const picks = ( (languages
            ...
            if (currentLanguageName === languageName) {
                description = ( localize(9578, "({0}) - Configured Language", languageId));
            }
            else {
                description = ( localize(9579, "({0})", languageId));
            }
            return {
                label: languageName,
                meta: extensions,
        ...
        picks.unshift({ type: 'separator', label: ( localize(9580, "languages (identifier)")) });
        ...
        if (hasLanguageSupport && resource) {
            ...
            configureLanguageSettings = { label: ( localize(9581, "Configure '{0}' language based settings...", currentLanguageName)) };
            picks.unshift(configureLanguageSettings);
            configureLanguageAssociations = { label: ( localize(9582, "Configure File Association for '{0}'...", ext)) };
            picks.unshift(configureLanguageAssociations);
        }
        const autoDetectLanguage = {
            label: ( localize(9583, "Auto Detect"))
        };
        picks.unshift(autoDetectLanguage);
        const pick = typeof languageMode === 'string' ? { label: languageMode } : await quickInputService.pick(picks, { placeHolder: ( localize(9584, "Select Language Mode")), matchOnDescription: true });
        if (!pick) {
            return;
        }
    ...
    configureFileAssociation(resource, languageService, quickInputService, configurationService) {
        ...
        const picks = ( (languages.map(({ languageName, languageId }) => {
            return {
                ...
                label: languageName,
                iconClasses: getIconClassesForLanguageId(languageId),
                description: (languageId === currentAssociation) ? ( localize(9585, "Current Association")) : undefined
            };
        })));
        setTimeout(async () => {
            const language = await quickInputService.pick(picks, { placeHolder: ( localize(9586, "Select Language Mode to Associate with '{0}'", extension || base)) });
            if (language) {
...
class ChangeEOLAction extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(9587, 'Change End of Line Sequence')),
            f1: true
        });
    }
    async run(accessor) {
        ...
        if (editorService.activeEditor?.isReadonly()) {
            await quickInputService.pick([{ label: ( localize(9548, "The active code editor is read-only.")) }]);
            return;
        }
        ...
        const eol = await quickInputService.pick(EOLOptions, { placeHolder: ( localize(9588, "Select End of Line Sequence")), activeItem: EOLOptions[selectedIndex] });
        if (eol) {
...
class ChangeEncodingAction extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(9589, 'Change File Encoding')),
            f1: true
        });
    }
    async run(accessor) {
        ...
        if (!encodingSupport) {
            await quickInputService.pick([{ label: ( localize(9590, "No file active at this time")) }]);
            return;
        }
        const saveWithEncodingPick = { label: ( localize(9591, "Save with Encoding")) };
        const reopenWithEncodingPick = { label: ( localize(9592, "Reopen with Encoding")) };
        ...
        if (encodingSupport instanceof UntitledTextEditorInput) {
            ...
            action = await quickInputService.pick([reopenWithEncodingPick, saveWithEncodingPick], { placeHolder: ( localize(9551, "Select Action")), matchOnDetail: true });
        }
        if (!action) {
            return;
        }
        ...
        if (guessedEncoding && configuredEncoding !== guessedEncoding && SUPPORTED_ENCODINGS[guessedEncoding]) {
            ...
            picks.unshift({ id: guessedEncoding, label: SUPPORTED_ENCODINGS[guessedEncoding].labelLong, description: ( localize(9593, "Guessed from content")) });
        }
        const encoding = await quickInputService.pick(picks, {
            placeHolder: isReopenWithEncoding ? ( localize(9594, "Select File Encoding to Reopen File")) : ( localize(9595, "Select File Encoding to Save with")),
...
export { ChangeEOLAction, ChangeEncodingAction, ChangeLanguageAction, EditorStatusContribution, ShowLanguageExtensionsAction };
