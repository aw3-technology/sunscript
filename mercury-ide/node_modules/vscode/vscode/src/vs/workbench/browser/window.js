...
import { getWindowsCount, getWindows, getWindow, getActiveWindow, getWindowById, addDisposableListener, EventType, EventHelper, windowOpenWithSuccess, windowOpenPopup, windowOpenNoOpener } from '../../base/browser/dom.js';
...
let BaseWindow = class BaseWindow extends Disposable {
    ...
    static async confirmOnShutdown(accessor, reason) {
        ...
        const message = reason === ShutdownReason.QUIT ?
            (isMacintosh ? ( localize(3536, "Are you sure you want to quit?")) : ( localize(3537, "Are you sure you want to exit?"))) :
            ( localize(3538, "Are you sure you want to close the window?"));
        const primaryButton = reason === ShutdownReason.QUIT ?
            (isMacintosh ? ( localize(3539, "&&Quit")) : ( localize(3540, "&&Exit"))) :
            ( localize(3541, "&&Close Window"));
        const res = await dialogService.confirm({
            message,
            primaryButton,
            checkbox: {
                label: ( localize(3542, "Do not ask me again"))
            }
        });
    ...
    onWillShutdown() {
        Event.toPromise(Event.any(Event.once(( (new DomEmitter(mainWindow.document.body, EventType.KEY_DOWN, true))).event), Event.once(( (new DomEmitter(mainWindow.document.body, EventType.MOUSE_DOWN, true))).event))).then(async () => {
            ...
            await this.dialogService.prompt({
                ...
                message: ( localize(3543, "An unexpected error occurred that requires a reload of this page.")),
                detail: ( localize(3544, "The workbench was unexpectedly disposed while running.")),
                buttons: [
                    {
                        label: ( localize(3545, "&&Reload")),
                ...
                if (matchesScheme(href, Schemas.http) || matchesScheme(href, Schemas.https)) {
                    ...
                    const showProtocolUrlOpenedDialog = async () => {
                        ...
                        await this.hostService.withExpectedShutdown(() => this.dialogService.prompt({
                            ...
                            message: ( localize(3553, "All done. You can close this tab now.")),
                            detail,
                            buttons,
                            cancelButton: true
                        }));
