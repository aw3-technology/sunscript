...
let GlobalCompositeBar = class GlobalCompositeBar extends Disposable {
    ...
    static { this.ACCOUNTS_ICON = registerIcon('accounts-view-bar-icon', Codicon.account, ( localize(6975, "Accounts icon in the view bar."))); }
    constructor(contextMenuActionsProvider, colors, activityHoverOptions, configurationService, instantiationService, storageService, extensionService) {
        ...
        this.globalActivityActionBar = this._register(( (new ActionBar(this.element, {
            actionViewItemProvider: (action, options) => {
                ...
                if (action.id === ACCOUNTS_ACTIVITY_ID) {
                    return this.instantiationService.createInstance(AccountsActivityActionViewItem, this.contextMenuActionsProvider, {
                        ...
                        actions.unshift(...[
                            toAction({ id: 'hideAccounts', label: ( localize(6976, "Hide Accounts")), run: () => setAccountsActionVisible(storageService, false) }),
                            ( (new Separator()))
                        ]);
                    });
                }
            ...
            ariaLabel: ( localize(6977, "Manage")),
    ...
    getContextMenuActions() {
        return [toAction({ id: 'toggleAccountsVisibility', label: ( localize(6978, "Accounts")), checked: this.accountsVisibilityPreference, run: () => this.accountsVisibilityPreference = !this.accountsVisibilityPreference })];
    }
...
let AccountsActivityActionViewItem = class AccountsActivityActionViewItem extends AbstractGlobalActivityActionViewItem {
    ...
    async resolveMainMenuActions(accountsMenu, disposables) {
        ...
        for (const providerId of providers) {
            if (!this.initialized) {
                const noAccountsAvailableAction = disposables.add(( (new Action('noAccountsAvailable', ( localize(6979, "Loading...")), undefined, false))));
            ...
            if (!accounts) {
                if (( (this.problematicProviders.has(providerId)))) {
                    const providerUnavailableAction = disposables.add(( (new Action('providerUnavailable', ( localize(6980, '{0} is currently unavailable', providerLabel)), undefined, false))));
            ...
            for (const account of accounts) {
                const manageExtensionsAction = toAction({
                    ...
                    label: ( localize(6981, "Manage Trusted Extensions")),
                    enabled: true,
        ...
        if (providers.length && !menus.length) {
            const noAccountsAvailableAction = disposables.add(( (new Action('noAccountsAvailable', ( localize(6983, "You are not signed in to any accounts")), undefined, false))));
            menus.push(noAccountsAvailableAction);
        }
...
function simpleActivityContextMenuActions(storageService, isAccount) {
    ...
    if (isAccount) {
        currentElementContextMenuActions.push(toAction({ id: 'hideAccounts', label: ( localize(6976, "Hide Accounts")), run: () => setAccountsActionVisible(storageService, false) }), ( (new Separator())));
    }
    return [
        ...currentElementContextMenuActions,
        toAction({ id: 'toggle.hideAccounts', label: ( localize(6978, "Accounts")), checked: isAccountsActionVisible(storageService), run: () => setAccountsActionVisible(storageService, !isAccountsActionVisible(storageService)) }),
        toAction({ id: 'toggle.hideManage', label: ( localize(6977, "Manage")), checked: true, enabled: false, run: () => { throw ( (new Error('"Manage" can not be hidden'))); } })
    ];
}
