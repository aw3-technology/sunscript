...
import { trackFocus, getWindow, isAncestorOfActiveElement } from '../../../../base/browser/dom.js';
...
let NotificationAccessibilityProvider = class NotificationAccessibilityProvider {
    ...
    getAriaLabel(element) {
        ...
        if (this._configurationService.getValue('accessibility.verbosity.notification')) {
            accessibleViewHint = keybinding ? ( localize(7331, "Inspect the response in the accessible view with {0}", keybinding)) : ( localize(
                7332,
        ...
        if (!element.source) {
            return accessibleViewHint ? ( localize(7333, "{0}, notification, {1}", element.message.raw, accessibleViewHint)) : ( localize(7334, "{0}, notification", element.message.raw));
        }
    ...
    getWidgetAriaLabel() {
        return this._options.widgetAriaLabel ?? ( localize(7337, "Notifications List"));
    }
    getRole() {
        return 'dialog';
    }
