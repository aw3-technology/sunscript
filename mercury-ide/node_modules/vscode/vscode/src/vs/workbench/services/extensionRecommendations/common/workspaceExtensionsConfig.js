...
import { isWorkspace } from '../../../../platform/workspace/common/workspace.js';
...
let WorkspaceExtensionsConfigService = class WorkspaceExtensionsConfigService extends Disposable {
    ...
    async toggleRecommendation(extensionId) {
        ...
        const workspaceOrFolders = isRecommended
            ? await this.pickWorkspaceOrFolders(recommendedWorksapceFolders, isWorkspaceRecommended ? workspace : undefined, ( localize(3847, "Remove extension recommendation from")))
            : await this.pickWorkspaceOrFolders(workspace.folders, workspace.configuration ? workspace : undefined, ( localize(3848, "Add extension recommendation to")));
    ...
    async toggleUnwantedRecommendation(extensionId) {
        ...
        const workspaceOrFolders = isUnwanted
            ? await this.pickWorkspaceOrFolders(unWantedWorksapceFolders, isWorkspaceUnwanted ? workspace : undefined, ( localize(3847, "Remove extension recommendation from")))
            : await this.pickWorkspaceOrFolders(workspace.folders, workspace.configuration ? workspace : undefined, ( localize(3848, "Add extension recommendation to")));
        for (const workspaceOrWorkspaceFolder of workspaceOrFolders) {
            if (isWorkspace(workspaceOrWorkspaceFolder)) {
    ...
    async pickWorkspaceOrFolders(workspaceFolders, workspace, placeHolder) {
        ...
        const folderPicks = ( (workspaceFolders.map(workspaceFolder => {
            return {
                ...
                description: ( localize(3849, "Workspace Folder")),
...
export { EXTENSIONS_CONFIG, WorkspaceExtensionsConfigService };
