...
const configurationExtPoint = ExtensionsRegistry.registerExtensionPoint({
    ...
    jsonSchema: {
        description: ( localize(683, 'Contributes extension defined themable colors')),
        type: 'array',
        items: {
            type: 'object',
            properties: {
                id: {
                    type: 'string',
                    description: ( localize(684, 'The identifier of the themable color')),
                ...
                description: {
                    type: 'string',
                    description: ( localize(686, 'The description of the themable color')),
                },
                defaults: {
                    type: 'object',
                    properties: {
        ...
        configurationExtPoint.setHandler((extensions, delta) => {
            for (const extension of delta.added) {
                ...
                if (!extensionValue || !Array.isArray(extensionValue)) {
                    collector.error(( localize(691, "'configuration.colors' must be a array")));
                    return;
                }
                ...
                for (const colorContribution of extensionValue) {
                    if (typeof colorContribution.id !== 'string' || colorContribution.id.length === 0) {
                        collector.error(( localize(693, "'configuration.colors.id' must be defined and can not be empty")));
                        return;
                    }
...
class ColorDataRenderer extends Disposable {
    ...
    render(manifest) {
        ...
        const headers = [
            ( localize(699, "ID")),
            ( localize(700, "Description")),
            ( localize(701, "Dark Default")),
            ( localize(702, "Light Default")),
            ( localize(703, "High Contrast Default")),
        ];
