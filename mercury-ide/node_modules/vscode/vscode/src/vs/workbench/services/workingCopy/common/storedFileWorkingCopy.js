...
import { ETAG_DISABLED, FileOperationResult, NotModifiedSinceFileOperationError } from '../../../../platform/files/common/files.js';
...
import { TaskSequentializer, raceCancellation, timeout } from '../../../../base/common/async.js';
import { ILogService } from '../../../../platform/log/common/log.service.js';
import { assertIsDefined } from '../../../../base/common/types.js';
import { IWorkingCopyFileService } from './workingCopyFileService.service.js';
import { IFilesConfigurationService } from '../../filesConfiguration/common/filesConfigurationService.service.js';
import { IWorkingCopyBackupService } from './workingCopyBackup.service.js';
...
import { IWorkingCopyEditorService } from './workingCopyEditorService.service.js';
import { IEditorService } from '../../editor/common/editorService.service.js';
import { IElevatedFileService } from '../../files/common/elevatedFileService.service.js';
...
let StoredFileWorkingCopy = class StoredFileWorkingCopy extends ResourceWorkingCopy {
    ...
    async doSave(options) {
        ...
        return this.progressService.withProgress({
            title: ( localize(10700, "Saving '{0}'", this.name)),
    ...
    doSaveSequential(versionId, options, progress, saveCancellation) {
        return this.saveSequentializer.run(versionId, (async () => {
            ...
            this.inErrorMode = false;
            progress.report({ message: ( localize(10701, "Writing into file...")) });
    ...
    doHandleSaveError(error, options) {
        ...
        if (fileOperationError.fileOperationResult === FileOperationResult.FILE_MODIFIED_SINCE) {
            ...
            primaryActions.push(toAction({ id: 'fileWorkingCopy.overwrite', label: ( localize(10703, "Overwrite")), run: () => this.save({ ...options, ignoreModifiedSince: true, reason: SaveReason.EXPLICIT }) }));
            primaryActions.push(toAction({ id: 'fileWorkingCopy.revert', label: ( localize(10704, "Discard")), run: () => this.revert() }));
        }
        else {
            ...
            if (canSaveElevated && (isPermissionDenied || triedToUnlock)) {
                primaryActions.push(toAction({
                    ...
                    label: triedToUnlock ?
                        isWindows ? ( localize(10705, "Overwrite as Admin...")) : ( localize(10706, "Overwrite as Sudo...")) :
                        isWindows ? ( localize(10707, "Retry as Admin...")) : ( localize(10708, "Retry as Sudo...")),
                    run: () => {
                ...
                primaryActions.push(toAction({ id: 'fileWorkingCopy.unlock', label: ( localize(10703, "Overwrite")), run: () => this.save({ ...options, writeUnlock: true, reason: SaveReason.EXPLICIT }) }));
            }
            else {
                primaryActions.push(toAction({ id: 'fileWorkingCopy.retry', label: ( localize(10709, "Retry")), run: () => this.save({ ...options, reason: SaveReason.EXPLICIT }) }));
            }
            primaryActions.push(toAction({
                id: 'fileWorkingCopy.saveAs',
                label: ( localize(10710, "Save As...")),
                run: async () => {
        ...
        const handle = this.notificationService.notify({ id: `${hash(( (this.resource.toString())))}`, severity: Severity$1.Error, message, actions: { primary: primaryActions } });
