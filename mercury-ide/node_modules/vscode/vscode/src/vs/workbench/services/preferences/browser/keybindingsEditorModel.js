...
import { compareIgnoreCase, equalsIgnoreCase } from '../../../../base/common/strings.js';
import { Language } from '../../../../base/common/platform.js';
import { or, matchesCamelCase, matchesWords, matchesContiguousSubString, matchesPrefix } from '../../../../base/common/filters.js';
import { UILabelProvider, AriaLabelProvider, UserSettingsLabelProvider } from '../../../../base/common/keybindingLabels.js';
...
import { isString, isEmptyObject } from '../../../../base/common/types.js';
...
const SOURCE_SYSTEM = ( localize(4636, "System"));
const SOURCE_EXTENSION = ( localize(4637, "Extension"));
const SOURCE_USER = ( localize(4638, "User"));
...
let KeybindingsEditorModel = KeybindingsEditorModel_1 = class KeybindingsEditorModel extends EditorModel {
    ...
    fetch(searchValue, sortByPrecedence = false) {
        ...
        if (commandIdMatches && commandIdMatches[1]) {
            ...
            return (
                 (filteredKeybindingItems.map(
                    keybindingItem => ({ id: KeybindingsEditorModel_1.getId(keybindingItem), keybindingItem, templateId: KEYBINDING_ENTRY_TEMPLATE_ID })
                ))
            );
        }
    ...
    static getCommandDefaultLabel(menuCommand) {
        if (!Language.isDefaultVariant()) {
            if (menuCommand && menuCommand.title && menuCommand.title.original) {
                ...
                return category ? ( localize(4639, "{0}: {1}", category, title)) : title;
            }
        }
        return null;
    }
...
class KeybindingItemMatches {
    ...
    matchesKeybinding(keybinding, searchValue, words, completeMatch) {
        ...
        if ((userSettingsLabel && compareIgnoreCase(searchValue, userSettingsLabel) === 0)
            || (ariaLabel && compareIgnoreCase(searchValue, ariaLabel) === 0)
            || (label && compareIgnoreCase(searchValue, label) === 0)) {
            return {
    ...
    wordMatchesAltModifier(word) {
        ...
        if (equalsIgnoreCase(( localize(4640, "option")), word)) {
            return true;
        }
        return false;
    }
    ...
    wordMatchesMetaModifier(word) {
        ...
        if (equalsIgnoreCase(( localize(4641, "meta")), word)) {
            return true;
        }
        return false;
    }
...
export { KEYBINDING_ENTRY_TEMPLATE_ID, KeybindingsEditorModel, createKeybindingCommandQuery };
