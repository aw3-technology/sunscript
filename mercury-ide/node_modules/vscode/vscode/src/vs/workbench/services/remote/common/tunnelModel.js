...
import { ProvidedOnAutoForward, isLocalhost, isAllInterfaces, TunnelProtocol, TunnelPrivacyId, LOCALHOST_ADDRESSES, ALL_INTERFACES_ADDRESSES } from '../../../../platform/tunnel/common/tunnel.js';
...
import { isString, isNumber, isObject } from '../../../../base/common/types.js';
...
const forwardedPortsViewEnabled = ( (new RawContextKey('forwardedPortsViewEnabled', false, ( localize(8676, "Whether the Ports view is enabled.")))));
...
const UserTunnelSource = {
    ...
    description: ( localize(8677, "User Forwarded"))
};
const AutoTunnelSource = {
    source: TunnelSource.Auto,
    description: ( localize(8678, "Auto Forwarded"))
};
...
let TunnelModel = class TunnelModel extends Disposable {
    ...
    makeLocalUri(localAddress, attributes) {
        if (localAddress.startsWith('http')) {
            return (
                 (URI.parse(localAddress))
            );
        }
        const protocol = attributes?.protocol ?? 'http';
        return (
             (URI.parse(`${protocol}://${localAddress}`))
        );
    }
            ...
            for (const tunnel of tunnels) {
                ...
                this.detected.set(makeAddress(tunnel.remoteAddress.host, tunnel.remoteAddress.port), {
                    ...
                    source: {
                        ...
                        description: ( localize(8680, "Statically Forwarded"))
                    }
                });
