...
let ViewQuickAccessProvider = class ViewQuickAccessProvider extends PickerQuickAccessProvider {
    ...
    constructor(viewDescriptorService, viewsService, outputService, terminalService, terminalGroupService, debugService, paneCompositeService, contextKeyService) {
        super(ViewQuickAccessProvider_1.PREFIX, {
            noResultsPick: {
                label: ( localize(4807, "No matching views")),
                containerLabel: ''
            }
        });
    ...
    doGetViewPickItems() {
        ...
        addPaneComposites(ViewContainerLocation.Sidebar, ( localize(4808, "Side Bar")));
        addPaneComposites(ViewContainerLocation.Panel, ( localize(4809, "Panel")));
        addPaneComposites(ViewContainerLocation.AuxiliaryBar, ( localize(4810, "Secondary Side Bar")));
        ...
        this.terminalGroupService.groups.forEach((group, groupIndex) => {
            group.terminalInstances.forEach((terminal, terminalIndex) => {
                const label = ( localize(4811, "{0}: {1}", `${groupIndex + 1}.${terminalIndex + 1}`, terminal.title));
                viewEntries.push({
                    label,
                    containerLabel: ( localize(4812, "Terminal")),
                    accept: async () => {
        ...
        this.debugService.getModel().getSessions(true).filter(s => s.hasSeparateRepl()).forEach((session, _) => {
            ...
            viewEntries.push({
                label,
                containerLabel: ( localize(4813, "Debug Console")),
                accept: async () => {
        ...
        for (const channel of channels) {
            viewEntries.push({
                label: channel.label,
                containerLabel: ( localize(4814, "Output")),
...
class OpenViewPickerAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(4815, 'Open View')),
...
class QuickAccessViewPickerAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(4816, 'Quick Open View')),
            category: Categories.View,
            f1: false,
            keybinding: {
