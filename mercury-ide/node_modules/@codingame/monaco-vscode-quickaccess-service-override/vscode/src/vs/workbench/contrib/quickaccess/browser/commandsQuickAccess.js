...
import { MenuId, MenuItemAction, Action2 } from 'vscode/vscode/vs/platform/actions/common/actions';
...
import { RelatedInformationType } from 'vscode/vscode/vs/workbench/services/aiRelatedInformation/common/aiRelatedInformation';
...
import { createKeybindingCommandQuery } from 'vscode/vscode/vs/workbench/services/preferences/browser/keybindingsEditorModel';
...
let CommandsQuickAccessProvider = class CommandsQuickAccessProvider extends AbstractEditorCommandsQuickAccessProvider {
    ...
    constructor(editorService, menuService, extensionService, instantiationService, keybindingService, commandService, telemetryService, dialogService, configurationService, editorGroupService, preferencesService, productService, aiRelatedInformationService, chatAgentService) {
        super({
            ...
            noResultsPick: () => ({
                label: ( localize(871, "No matching commands")),
                commandId: ''
            }),
        ...
        return (
             ([
                ...
                buttons: [{
                        ...
                        tooltip: ( localize(872, "Configure Keybinding")),
                    }],
                trigger: () => {
    ...
    async getAdditionalCommandPicks(allPicks, picksSoFar, filter, token) {
        ...
        if (defaultAgent) {
            additionalPicks.push({
                label: ( localize(873, "Ask {0}: {1}", defaultAgent.fullName, filter)),
    ...
    getGlobalCommandPicks() {
        ...
        for (const action of globalCommandsMenuActions) {
            ...
            if (category) {
                label = ( localize(874, "{0}: {1}", category, label));
            }
...
class ShowAllCommandsAction extends Action2 {
    ...
    constructor() {
        super({
            ...
            title: ( localize2(875, 'Show All Commands')),
            keybinding: {
...
class ClearCommandHistoryAction extends Action2 {
    constructor() {
        super({
            ...
            title: ( localize2(876, 'Clear Command History')),
            f1: true
        });
    }
    async run(accessor) {
        ...
        if (commandHistoryLength > 0) {
            const { confirmed } = await dialogService.confirm({
                type: 'warning',
                message: ( localize(877, "Do you want to clear the history of recently used commands?")),
                detail: ( localize(878, "This action is irreversible!")),
                primaryButton: ( localize(879, "&&Clear"))
            });
            if (!confirmed) {
                return;
            }
