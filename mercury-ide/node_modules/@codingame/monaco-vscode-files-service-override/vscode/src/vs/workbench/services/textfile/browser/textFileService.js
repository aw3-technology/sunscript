...
let AbstractTextFileService = class AbstractTextFileService extends Disposable {
    ...
    static { this.TEXTFILE_SAVE_CREATE_SOURCE = SaveSourceRegistry.registerSource('textFileCreate.source', ( localize(4399, "File Created"))); }
    static { this.TEXTFILE_SAVE_REPLACE_SOURCE = SaveSourceRegistry.registerSource('textFileOverwrite.source', ( localize(4400, "File Replaced"))); }
    ...
    provideDecorations() {
        const provider = this._register(new (class extends Disposable {
            constructor(files) {
                super();
                this.files = files;
                this.label = ( localize(4401, "Text File Model Decorations"));
            ...
            provideDecorations(uri) {
                ...
                if (isReadonly && isOrphaned) {
                    return {
                        ...
                        strikethrough: true,
                        tooltip: ( localize(4402, "Deleted, Read-only")),
                    };
                }
                else if (isReadonly) {
                    return {
                        letter: Codicon.lockSmall,
                        tooltip: ( localize(4403, "Read-only")),
                    };
                }
                else if (isOrphaned) {
                    return {
        ...
        try {
            ...
            if (error.decodeStreamErrorKind === DecodeStreamErrorKind.STREAM_IS_BINARY) {
                throw ( (new TextFileOperationError(
                    localize(4405, "File seems to be binary and cannot be opened as text"),
    ...
    async confirmOverwrite(resource) {
        const { confirmed } = await this.dialogService.confirm({
            ...
            primaryButton: ( localize(4408, "&&Replace")),
        });
        return confirmed;
    }
    async confirmMakeWriteable(resource) {
        const { confirmed } = await this.dialogService.confirm({
            ...
            detail: ( localize(4410, "Paths can be configured as read-only via settings.")),
            primaryButton: ( localize(4411, "&&Save Anyway"))
        });
        return confirmed;
    }
