...
// Application Services (Client-side only)
...
container.bind(TYPES.IDEService).to(ClientIDEService).inSingletonScope();
...
// Bind Contributions (with singleton guard)
...
if (!container.isBound(TYPES.MenuContribution)) {
...
if (!container.isBound(TYPES.KeybindingContribution)) {
